...
Public Class Frm_PosicaoPortadorPaga

    Private Const _valorZERO As Integer = 0
    Private Mlojas As String
    Public Shared _frmRef As New Frm_PosicaoPortadorPaga
    ...
    Private _sImpressao As Cl_EscreveArquivo
    Private _leitorTabelaImprimir As NpgsqlDataReader
    Dim MostrarCaixaImpressoras As Boolean = False

    Private Sub Frm_pagPortador_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub Frm_pagPortador_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
    ...
    Private Sub consultaBD(ByVal CodPort As String, ByVal IndiceX As Integer)
        ...
        Try
            ...
            Select Case IndiceX
                ...
                Case 0
                    sqlPart.Append("SELECT d_geno AS ""Loja"", d_tipo AS ""TP"", d_sit AS ""SIT"", ") '2
                    sqlPart.Append("d_banco AS ""Banco"",d_duplic AS ""Documento"", d_emiss AS ""DtEmissão"", d_vencto AS ""DtVencto"",d_dtpaga AS ""DtPaga"", d_juros AS ""Juros R$ "", ") '5
                    sqlPart.Append("d_valor AS ""Valor R$"" FROM fatp001 Fp WHERE d_portad = '" & RTrim(txt_codPart.Text) & "' AND d_sit = 'N' AND d_geno = '" & Mlojas & "' ") '

                    'cmdTot.Append("SELECT sum(d_valor) FROM fatd001 where d_geno='G00" & Frm_Dup_ManDuplicatas.Frm_ManDuplicadas.cbo_loja.SelectedItem & "' and d_portad='" & RTrim(txt_codPart.Text) & "' and d_sit='N' or d_sit='D' ")

                Case 1
                    sqlPart.Append("SELECT d_geno AS ""Loja"", d_tipo AS ""TP"", d_sit AS ""SIT"", ") '2
                    sqlPart.Append("d_banco AS ""Banco"",d_duplic AS ""Documento"", d_emiss AS ""DtEmissão"", d_vencto AS ""DtVencto"",d_dtpaga AS ""DtPaga"", d_juros AS ""Juros R$ "", ") '5
                    sqlPart.Append("d_valor AS ""Valor R$"" FROM fatp001 Fp WHERE d_geno = '" & Mlojas & "' AND d_portad = '" & RTrim(txt_codPart.Text) & "' AND d_sit = 'L' ") '

                Case 2
                    sqlPart.Append("SELECT d_geno AS ""Loja"", d_tipo AS ""TP"", d_sit AS ""SIT"", ") '2
                    sqlPart.Append("d_banco AS ""Banco"",d_duplic AS ""Documento"", d_emiss AS ""DtEmissão"", d_vencto AS ""DtVencto"",d_dtpaga AS ""DtPaga"", d_juros AS ""Juros R$ "", ") '5
                    sqlPart.Append("d_valor AS ""Valor R$"" FROM fatp001 Fp WHERE d_geno = '" & Mlojas & "' AND d_portad = '" & RTrim(txt_codPart.Text) & "' AND d_sit = 'D' ") '

                    'cmdTot.Append("SELECT sum(d_valor) FROM fatd001 where  d_geno='G00" & Frm_Dup_ManDuplicatas.Frm_ManDuplicadas.cbo_loja.SelectedItem & "' and  d_portad='" & RTrim(txt_codPart.Text) & "' and d_sit='N' or d_sit='D'")

                Case 3
                    sqlPart.Append("SELECT d_geno AS ""Loja"", d_tipo AS ""TP"", d_sit AS ""SIT"", ") '2
                    sqlPart.Append("d_banco AS ""Banco"", d_duplic AS ""Documento"", d_emiss AS ""DtEmissão"", ") '5
                    sqlPart.Append("d_vencto AS ""DtVencto"", d_dtpaga AS ""DtPaga"", d_juros AS ""Juros R$"", ") '
                    sqlPart.Append("d_valor AS ""Valor R$"" ")
                    sqlPart.Append("FROM fatp001 Fp WHERE ")
                    sqlPart.Append("Fp.d_geno = '" & Mlojas & "' AND Fp.d_portad = '" & Trim(Me.txt_codPart.Text) & "' ")


                    cmdTot.Append("SELECT sum(d_valor) FROM fatp001 where d_geno='G00" & Frm_ManPagamentos.Frm_ManPagamentos._loja00 & "' and d_portad='" & RTrim(txt_codPart.Text) & "' and d_sit='N' or d_sit='D'")
                ...
                Case 4
                    sqlPart.Append("SELECT d_geno AS ""Loja"", d_tipo AS ""TP"", d_sit AS ""SIT"", ") '2
                    sqlPart.Append("d_banco AS ""Banco"",d_duplic AS ""Documento"", d_emiss AS ""DtEmissão"", d_vencto AS ""DtVencto"",d_dtpaga AS ""DtPaga"", d_juros AS ""Juros R$ "", ") '5
                    sqlPart.Append("d_valor AS ""Valor R$"" FROM fatp001 Fp WHERE d_geno = '" & Mlojas & "' AND d_portad = '" & RTrim(txt_codPart.Text) & "' AND d_sit = 'N' AND d_vencto < CURRENT_DATE ") '

                Case 5
                    sqlPart.Append("SELECT d_geno AS ""Loja"", d_tipo AS ""TP"", d_sit AS ""SIT"", ") '2
                    sqlPart.Append("d_banco AS ""Banco"",d_duplic AS ""Documento"", d_emiss AS ""DtEmissão"", d_vencto AS ""DtVencto"",d_dtpaga AS ""DtPaga"", d_juros AS ""Juros R$ "", ") '5
                    sqlPart.Append("d_valor AS ""Valor R$"" FROM fatp001 Fp WHERE d_geno = '" & Mlojas & "' AND d_portad = '" & RTrim(txt_codPart.Text) & "' AND d_sit = 'E' ") '

            End Select


            If cbo_tipo.SelectedIndex > 0 Then
                sqlPart.Append("AND Fp.d_tipo = '" & Mid(cbo_tipo.SelectedItem.ToString, 1, 2) & "' ")
            End If

            sqlPart.Append("ORDER BY d_emiss")
    ...
    Public Function trazCliente(ByVal codCli As String) As Boolean
        ...
        Dim codigo, nome, cpd_cnpj, inscricao, UF As String
        codigo = "" : nome = "" : cpd_cnpj = "" : inscricao = "" : UF = ""
        ...
        codigo = Nothing : nome = Nothing : cpd_cnpj = Nothing : inscricao = Nothing : UF = Nothing
    ...
    Private Sub Frm_pagPortador_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load


        lbl_NomeSys.Text = Application.ProductName
        If Frm_ManPagamentos.Frm_ManPagamentos._loja00.Equals("") Then

            Mlojas = MdlEmpresaUsu._codigo
        Else
            Mlojas = "G00" & Frm_ManPagamentos.Frm_ManPagamentos._loja00
        End If
    ...
    Private Sub executaRelatorio(ByVal unidadePC As String, ByVal arqSaida As String)
        ...
        Try
            ...
            s.EscreveLn(strLinha & vbNewLine)

            's.EscreveLn(_clFuncoes.Exibe_Str(Me.txt_nomePart.Text, 100))
    ...
    Private Sub GravCabecalhoCliente(ByVal s As Cl_EscreveArquivo, ByVal codCliente As String)
        ...
        Try
            ...
            If cbo_opcoes.SelectedIndex = 3 Then
                ...
                s.EscreveLn(_clFuncoes.Exibe_Str(strLinha, 106))
            ...
            Else
                ...
                End If
                strLinha += _clFuncoes.Exibe_StrDireita("Emis: " & Format(Date.Now, "dd/MM/yyyy"), 20) & " "
                s.EscreveLn(_clFuncoes.Exibe_Str(strLinha, 85))
    ...
    Private Sub gravaDuplicadasExtracted(ByVal s As Cl_EscreveArquivo)
        ...
        Select Case cbo_opcoes.SelectedIndex
            ...
            Case 3
                ...
                '            123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890
                s.EscreveLn("----------------------------------------------------------------------------------------------------------")
                s.EscreveLn("  LOJA  TP SIT BANCO DOCUMENTO  DtEmiss    DtVencto   DtPaga      Juros   ValorR$   ParcialR$  DtParcial ")
            ...
            Case Else
                ...
                '            123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890
                s.EscreveLn("-------------------------------------------------------------------------------------")
                s.EscreveLn("  LOJA  TP SIT BANCO DOCUMENTO  DtEmiss    DtVencto   DtPaga      Juros   ValorR$ ")
                '              xxxxx xx  x  xxxxx xxxxxxxxxZ xxxxxxxxxZ xxxxxxxxxZ xxxxxxxxxZ  xxxxx xxxxxxxxxZ xxxxxxxxxZ xxxxxxxxxZ
                s.EscreveLn("  --------------------------------------------------------------------------------")
    ...
    Private Sub gravaDuplicadas(ByVal s As Cl_EscreveArquivo)
        ...
        Try
            ...
            Select Case Me.cbo_opcoes.SelectedIndex
                ...
                Case 3
                    ...
                    sqlPart.Append("d_banco AS ""Banco"", d_duplic AS ""Documento"", d_emiss AS ""DtEmissão"", ") '5
                    sqlPart.Append("d_vencto AS ""DtVencto"", d_dtpaga AS ""DtPaga"", d_juros AS ""Juros R$"", ") '
                    sqlPart.Append("d_valor AS ""Valor R$"" FROM fatp001 Fp WHERE ")
                    sqlPart.Append("d_geno = '" & Mlojas & "' AND d_portad = '" & Trim(Me.txt_codPart.Text) & "' ")
            ...
            While drPart.Read
                    ...
                    Case 3
                        If (mContPg = 1) AndAlso (mContRegistrosPg = 45) Then
                            ...
                            Select Case cbo_opcoes.SelectedIndex
                                ...
                                Case 3
                                    s.EscreveLn("                    C O N T I N U A C A O . . .                                                 Pg.: " & String.Format("{0:D3}", mContPg))
                                    s.EscreveLn("")

                                Case Else
                                    s.EscreveLn("                    C O N T I N U A C A O . . .                                   " & String.Format("{0:D3}", mContPg))
                                    s.EscreveLn("")
                ...
                Try
                    mDtemiss = Format(drPart(5), "dd/MM/yyyy")
                Catch ex As Exception
                    mDtemiss = ""
                End Try

                Try
                    mDtVencto = Format(drPart(6), "dd/MM/yyyy")
                Catch ex As Exception
                    mDtVencto = ""
                End Try
                ...
                s.EscreveLn(_clFuncoes.Exibe_Str(strLinha, 104))
        ...
        If mContItens > _valorZERO Then
            ...
            End If
            s.EscreveLn(_clFuncoes.Exibe_Str(strLinha, 115))

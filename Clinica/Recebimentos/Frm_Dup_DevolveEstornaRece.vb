...
Public Class Frm_Dup_DevolveEstornaRece
    ...
    Private Sub Estorno_Dev_Dup(ByVal mDocumento As String, ByVal IndiceX As Integer)
        ...
        Try
            ...
            strSQL.Append("f_dtpaga=@f_dtpaga, f_juros=@f_juros, f_desc=@f_desc, f_sit=@f_sit, f_sitanterior = @f_sitanterior ")
            strSQL.Append("WHERE f_duplic='" & mDocumento.ToString & "' and f_geno='" & _geno001.pCodig & "'")

            oCmd = New NpgsqlCommand(strSQL.ToString, oConn)
            Juros = 0.0 : Desconto = 0.0

            oCmd.Parameters.Add("@f_dtpaga", Convert.ChangeType(MdlConexaoBD.dataServidor, GetType(Date)))
            oCmd.Parameters.Add("@f_juros", Juros)
            oCmd.Parameters.Add("@f_desc", Desconto)
            oCmd.Parameters.Add("@f_sitanterior", mSit)
            If IndiceX = 0 Then
                oCmd.Parameters.Add("@f_sit", "E")
            Else
                oCmd.Parameters.Add("@f_sit", "D")
            End If
    ...
    Private Sub txt_documento_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_documento.Leave
        ...
        If txt_documento.Text.Equals("") = False Then
            ...
            Sqlcomm.Append("Select F.f_geno,F.f_duplic,F.f_valor,F.f_portad,C.p_portad,C.p_cod ,F.f_vencto, F.f_sit FROM " & _geno001.pEsquemaestab & ".fatd001 F, Cadp001 C WHERE ") '5
            Sqlcomm.Append("F.f_duplic='" & Me.txt_documento.Text & "' AND F.f_portad=C.p_cod AND F.f_geno='" & _geno001.pCodig & "'")

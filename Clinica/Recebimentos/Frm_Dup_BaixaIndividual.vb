...
Public Class Frm_Dup_BaixaIndividual
    ...
    Private Sub Frm_Dup_BaixaIndividual_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            _idRecebimento = formManDuplicRecebimento._idRecebimento
    ...
    Private Sub executaF6()
        ...
        executaEspelhoNF_R("", "\wged\reletorios\reciboIndividual.TXT")
    End Sub
            ...
            If txt_valor.Text <> "0,00" Then
                ...
                If MessageBox.Show("Confirma Baixa de Documento !", "Quitação ", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    ElseIf CDbl(Me.txt_valor.Text) < CDbl(Me.txt_subtotal.Text) Then
                        ...
                        cl_bd.IncDuplicatas_Parciais(_geno001.pCodig, mCdport, MTipo, Mid(txt_documento.Text, 1, 9), Mid(txt_documento.Text, 1, 9), "", _
    ...
    Private Sub Baixa_dupIndividual_Total(ByVal Ndupl As String, ByVal TipoPgto As String, ByVal DTPaga As Date, ByVal Juros As Double, _
        ...
        Try
            strSQL.Append("UPDATE " & _geno001.pEsquemaestab & ".fatd001 SET ")
            strSQL.Append("f_dtpaga = @f_dtpaga, f_juros = @f_juros, f_desc = @f_desc, f_sit = @f_sit, f_sitanterior = @f_sitanterior WHERE ")
            If _idRecebimento > 0 Then
                strSQL.Append("f_idx=" & _idRecebimento)
            Else
                strSQL.Append("f_duplic='" & Ndupl.ToString & "' AND f_geno='" & _geno001.pCodig & "'")
            End If
            ...
            oCmd.Parameters.Add("@f_sitanterior", "N")
    ...
    Private Sub Baixa_dupIndividual_Parcial(ByVal CdPort As String, ByVal Ndupl As String, ByVal SaldoConta As Double, ByVal conexao As String)
        ...
        Dim strSQL As New StringBuilder
        Try
            strSQL.Append("UPDATE " & _geno001.pEsquemaestab & ".fatd001 SET f_valor=@f_valor WHERE ")
            If _idRecebimento > 0 Then
                strSQL.Append("f_idx=" & _idRecebimento)
            Else
                strSQL.Append("f_geno='" & _geno001.pCodig & "' AND f_portad=f_portad AND ")
    ...
    Private Sub trazValoresDocumento()
        ...
        If txt_documento.Text <> "" Then
            ...
            Sqlcomm.Append(_geno001.pEsquemaestab & ".fatd001 F LEFT JOIN Cadp001 C ON C.p_cod = F.f_portad  WHERE ") '5
            If _idRecebimento > 0 Then
                Sqlcomm.Append("F.f_idx=" & _idRecebimento)
            Else
                Sqlcomm.Append("F.f_duplic='" & Me.txt_documento.Text & "' AND F.f_portad=C.p_cod AND F.f_geno='" & _geno001.pCodig & "' LIMIT 1")
            End If
            ...
            Dim mExistRow As Boolean = False
            ...
            Try
                ...
                If DrDup.HasRows Then mCdport = "" : mExistRow = True
                While (DrDup.Read())
                ...
                End While
                If mExistRow Then _clFunc.trazCadp001(mCdport, _cliente)
            Catch ex As Exception
                MsgBox(ex.Message.ToString)
            End Try
        Else
    ...
    Private Sub executaEspelhoNF_R(ByVal unidadePC As String, ByVal arqSaida As String)
        ...
        GravCabecalhoLoja(s, _geno001.pCodig, lShouldReturn1)
    ...
    Private Sub rptGravaTotaisNF(ByVal sender As Object, ByVal Relatorio As System.Drawing.Printing.PrintPageEventArgs)
        ...
        'Cabecalho...
        Relatorio.Graphics.DrawString(_geno001.pGeno, fonteTitulo, Brushes.Black, margemEsq + 180, 70, New StringFormat())
        Relatorio.Graphics.DrawString("Endereço: " & _geno001.pEnder, fonteCabecalho, Brushes.Black, margemEsq + 180, 90, New StringFormat())
        Relatorio.Graphics.DrawString("Cidade: " & _geno001.pCid & " - " & _geno001.pUf, fonteCabecalho, Brushes.Black, margemEsq + 180, 110, New StringFormat())
        Relatorio.Graphics.DrawString("CEP: " & _geno001.pCep, fonteCabecalho, Brushes.Black, margemEsq + 180, 130, New StringFormat())
        ...
        strLinha = _clFunc.primeiraLetraMaiusculaPalavra(_geno001.pCid) & " (" & _geno001.pUf & "), " & Date.Now.Day & " de " & _clFunc.returnMesExtenso(Date.Now.Month) & " de " & Date.Now.Year

...
Public Class Frm_relatFinanReceb
    ...
    Dim _somaVlrParcial, _somaVlrDupl As Double
    Dim _duplLiquidada As Boolean = False
    Dim _numDupl As String = ""
    ...
    Private Sub Frm_relatFinanReceb_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Me.cbo_loja.SelectedIndex = _clFuncoes.trazIndexCboLoja2dig(Mid(MdlUsuarioLogando._local, MdlUsuarioLogando._local.Length - 1, 2), cbo_loja)
    ...
    Private Sub executaF6()
        ...
        executaRelatorio("", "\wged\relatorios\consultaFinancRec.txt")
    End Sub
    ...
    Private Sub executaRelatorio(ByVal unidadePC As String, ByVal arqSaida As String)
        ...
        Dim mArqTemp As String = "\wged\relatorios\TEMPconsultaFinancRec.TMP"
    ...
    Private Sub gravaFinanceiro()
        ...
        Try
            ...
            Select Case Me.cbo_opcoes.SelectedIndex
                ...
                Case 0 'Doc. Pagos no Período
                    ...
                    sqlFinan.Append(MdlEmpresaUsu._esqEstab & ".fatdp02 Pc ON ft.f_duplic = Pc.f_duplic WHERE cad.p_cod = Ft.f_portad AND Ft.f_sit = 'L' AND (Ft.f_dtpaga BETWEEN '" & Format(dtp_inicial.Value, "dd/MM/yyyy") & "' AND '" & Format(dtp_final.Value, "dd/MM/yyyy") & "') ")
                ...
                Case 1 'Doc. Vencidas por Carteira
                    ...
                    sqlFinan.Append("FROM " & MdlEmpresaUsu._esqEstab & ".fatd001 Ft LEFT JOIN cadp001 cad ON cad.p_cod = f_portad WHERE Ft.f_vencto < CURRENT_DATE AND f_sit = 'N' AND f_cartei = '" & Mid(Me.cbo_carteira.SelectedItem.ToString, 1, 2) & "' ORDER BY Ft.f_vencto ASC") '

                Case 2 'Doc. a Vencer por Período
                    sqlFinan.Append("SELECT f_geno AS ""Loja"", f_tipo AS ""TP"", f_sit AS ""SIT"", ") '2
                    sqlFinan.Append("f_banco AS ""Banco"",f_duplic AS ""Documento"", f_emiss AS ""DtEmissão"", f_vencto AS ""DtVencto"", ") '6
                    sqlFinan.Append("f_dtpaga AS ""DtPaga"", f_juros AS ""Juros R$ "", f_valor AS ""Valor R$"", cad.p_portad AS ""Portador"" ") '10
                    sqlFinan.Append("FROM " & MdlEmpresaUsu._esqEstab & ".fatd001 Ft LEFT JOIN cadp001 cad ON cad.p_cod = f_portad WHERE f_sit = 'N' AND Ft.f_vencto >= CURRENT_DATE AND (Ft.f_vencto BETWEEN '" & Format(dtp_inicial.Value, "dd/MM/yyyy") & "' AND '" & Format(dtp_final.Value, "dd/MM/yyyy") & "') ")
                ...
                Case 3 'Doc. Incluidas por Dia
                    ...
                    sqlFinan.Append("Ft.f_valor AS ""Valor R$"", Pc.f_valor AS ""Parcial R$"", Pc.f_dtpaga AS ""DtPaga "", cad.p_portad AS ""Portador"" ") '12
                    sqlFinan.Append("FROM cadp001 cad, " & MdlEmpresaUsu._esqEstab & ".fatd001 Ft LEFT OUTER JOIN ")
                    sqlFinan.Append(MdlEmpresaUsu._esqEstab & ".fatdp02 Pc ON Ft.f_duplic = Pc.f_duplic ") ' OR Ft.f_dtpaga = '" & Format(MdlConexaoBD.dataServidor, "dd/MM/yyyy") & "')
                    sqlFinan.Append("WHERE cad.p_cod = Ft.f_portad AND Ft.f_emiss = '" & Format(MdlConexaoBD.dataServidor, "dd/MM/yyyy") & "' ")
                ...
                Case 4 'Dupl.Pagas - Pendente
                    ...
                    sqlFinan.Append("Ft.f_valor AS ""Valor R$"", Pc.f_valor AS ""Parcial R$"", Pc.f_dtpaga AS ""DtPaga "", cad.p_portad AS ""Portador"" ") '12
                    sqlFinan.Append("FROM cadp001 cad, " & MdlEmpresaUsu._esqEstab & ".fatd001 Ft LEFT OUTER JOIN ")
                    sqlFinan.Append(MdlEmpresaUsu._esqEstab & ".fatdp02 Pc ON Ft.f_duplic = Pc.f_duplic ")
                    sqlFinan.Append("WHERE cad.p_cod = Ft.f_portad AND Ft.f_sit = 'N' AND Ft.f_cartei = '01' AND (Ft.f_dtpaga BETWEEN '" & Format(dtp_inicial.Value, "dd/MM/yyyy") & "' AND '" & Format(dtp_final.Value, "dd/MM/yyyy") & "') ")
            ...
            Select Case cbo_opcoes.SelectedIndex
                Case 0, 3, 4 'Doc. Pagos no Período
            ...
            _somaVlrDupl = 0 : _somaVlrParcial = 0 : _duplLiquidada = False : _numDupl = "NUMERO DUPLIC"
            ...
            While drFinan.Read
                ...
                Try
                    mDtemiss = Format(Convert.ChangeType(drFinan(5), GetType(Date)), "dd/MM/yyyy")
                Catch ex As Exception
                    mDtemiss = ""
                End Try

                Try
                    mDtVencto = Format(Convert.ChangeType(drFinan(6), GetType(Date)), "dd/MM/yyyy")
                Catch ex As Exception
                    mDtVencto = ""
                End Try

                Try
                    mDtPagNormal = Format(Convert.ChangeType(drFinan(7), GetType(Date)), "dd/MM/yyyy")
                Catch ex As Exception
                    mDtPagNormal = ""
                End Try
                ...
                Try
                    ...
                    Try
                        mDtPaga = Format(Convert.ChangeType(drFinan(11), GetType(Date)), "dd/MM/yyyy")
                    Catch ex As Exception
                        mDtPaga = ""
                    End Try
                Catch ex As Exception
                ...
                If IsDBNull(drFinan(10)) = False Then _somaVlrParcial += drFinan(10)

                If mSit.Equals("L") Then

                    If (_numDupl.Substring(0, _numDupl.Length - 1).Equals(mDocumento.Substring(0, mDocumento.Length - 1)) = False) Then
                        _somaVlrDupl += drFinan(9) : _numDupl = mDocumento
                    End If
                Else
                    _somaVlrDupl += drFinan(9)
                End If
        ...
        sqlFinan.Remove(_valorZERO, sqlFinan.ToString.Length) : oConnBD.Close()
        ...
        If mContItens > _valorZERO Then
            ...
            Select Case cbo_opcoes.SelectedIndex
                ...
                Case 0, 3, 4
                    '                       1         2         3         4         5         6         7         8         9         0         1         2         3         4         5
                    '              123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890
                    _strLinha += _clFuncoes.Exibe_StrDireita(Format(Round(_somaVlrDupl, 2), "###,##0.00"), 79)
                    If _somaVlrParcial > 0 Then _strLinha += _clFuncoes.Exibe_StrDireita(Format(Round(_somaVlrParcial, 2), "###,##0.00"), 11)
                    _sw.EscreveLn(_clFuncoes.Exibe_Str(_strLinha, 126))
                    _sw.EscreveLn("  -----------------------------------------------------------------------------------------------------------------------------------")

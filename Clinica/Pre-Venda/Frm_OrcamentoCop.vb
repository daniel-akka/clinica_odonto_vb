...
Public Class Frm_OrcamentoCop
    ...
    Dim mNumPedidoTemp As String = ""
    Dim _numOrcamento As String = ""
    ...
    Public Shared _frmREf As New Frm_OrcamentoCop
    ...
    Private _aliqDescProdEditado, _vlrProdEditado As Double
    ...
    Public ClfProd_Ref, FilialProd_Ref, cdBarraProd_Ref As String
    ...
    Private Sub Frm_OrcamentoCop_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub Frm_OrcamentoCop_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
    ...
    Private Sub Frm_OrcamentoCop_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        cbo_condpgto = _clFuncoes.PreenchComboCondPagto(cbo_condpgto, MdlConexaoBD.conectionPadrao)
        ...
        _numPedidoOK = True
        _numOrcamento = _FrmGeraOrcamento._frmRefGeraOrcamento._numOrcamento
        _mapaPedido = _FrmGeraOrcamento._frmRefGeraOrcamento._mapaPedido
        ...
        'Pega o próximo numero do pedido...
        ...
        Dim mProxNumPedido As Integer = 0
        'Numero do pedido...
        txt_pedido.Text = _clBD.trazProxNumPedido(MdlEmpresaUsu._codigo, conexao)
        mNumPedido = txt_pedido.Text : mNumPedidoTemp = txt_pedido.Text : _numPedidoTemp = mNumPedidoTemp
        mProxNumPedido = CInt(txt_pedido.Text) + 1
        _clBD.updateGenp001NumPedido(MdlEmpresaUsu._codigo, String.Format("{0:D8}", mProxNumPedido), conexao, transacao)
        transacao.Commit() : conexao.ClearAllPools() : conexao.Close() : conexao = Nothing


        'Executando o pedido na tabela temporária...
        _numPedidoOK = True
        Try
            _fsnumpedido = New FileStream(_arqNumPedido, FileMode.Create, FileAccess.ReadWrite)
            _snumpedido = New StreamWriter(_fsnumpedido)
            _snumpedido.Write(txt_pedido.Text)
            _snumpedido.Close() : _fsnumpedido.Close()

        Catch ex As Exception
            MsgBox("ERRO ao gravar numero do pedido no arquivo:: " & ex.Message, MsgBoxStyle.Exclamation)
        End Try

        executaIncluiOrca1Orca4TblTemp()
    ...
    Private Sub msk_senha_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles msk_senha.Leave
        ...
        If msk_senha.Text <> "" Then
            ...
            If _clBD.existeGerente(oConnBDMETROSYS, mnomeGerente, msenhaGerente) Then
                ...
                ElseIf _clBD.trazLibValorGerente(oConnBDMETROSYS, mnomeGerente, msenhaGerente) Then
    ...
    Private Sub cbo_condpgto_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbo_condpgto.Leave
        ...
        If cbo_condpgto.SelectedIndex >= _valorZERO Then
            ...
            preencheCondicoesPagto(Me.cbo_condpgto.SelectedItem)
        End If


    End Sub

    Private Sub preencheCondicoesPagto(ByVal condicoes As String)
        ...
        Try
            ...
            End While
            drCondPagto.Close() : drCondPagto = Nothing
        ...
        sqlCondPagto = Nothing : cmdCondPagto = Nothing
        oConnBDMETROSYS.ClearPool() : oConnBDMETROSYS.Close() : oConnBDMETROSYS = Nothing
    End Sub
        ...
        If (e.KeyCode = Keys.Enter) OrElse (e.KeyCode = Keys.Tab) Then
            ...
            If Me.txt_codProd.Text.Equals("") Then
                ...
                Try
                    ...
                    Me.txt_valor.Text = ValorUnitProd_Ref
    ...
    Public Function trazItenBD(ByVal codIten As String) As Boolean
        ...
        SqlProduto = Nothing : CmdProduto = Nothing
        oConnBDGENOV.ClearPool() : oConnBDGENOV.Close() : oConnBDGENOV = Nothing
    ...
    Public Function trazItenFilialBD(ByVal codIten As String) As Boolean
        ...
        CmdProduto.CommandText = "" : SqlProduto.Remove(_valorZERO, SqlProduto.ToString.Length)
        CmdProduto = Nothing : SqlProduto = Nothing
        oConnBDGENOV.ClearPool() : oConnBDGENOV.Close() : oConnBDGENOV = Nothing
    ...
    Private Sub addItemEditadoGrid()
        ...
        Try
            ...
            Dim mvlUnitVendPorItem, mvlSubPorItem, mvldescPorItem, mvlUnitPorItem As Double
            ...
            If malqdesc > _valorZERO Then
                ...
                mvlUnitPorItem = mvlUnitVendPorItem
            Else
            ...
            Dim mlinha As String() = {_valorZERO, Mid(Me.cbo_local.SelectedItem, 1, 2), codProd_Ref, cdBarraProd_Ref, _
                                      ...
                                      malqdesc, mvldesc, FilialProd_Ref, mbaseicms, moutrasdesp, mvlcims, mvlUnitPorItem}
            ...
            If txt_codProd.Text.Equals(_codProdEditando) Then
                ...
                If CDbl(txt_qtde.Text) <> _qtdeAnteriorProd Then
                    ...
                    If CDbl(txt_qtde.Text) < _qtdeAnteriorProd Then
                        ...
                        _clBD.somaQtdeProd(txt_codProd.Text, FilialProd_Ref, mNovaQtde, conection, transacao)

                    ElseIf CDbl(txt_qtde.Text) > _qtdeAnteriorProd Then
                        mNovaQtde = CDbl(txt_qtde.Text) - _qtdeAnteriorProd
                        _clBD.subtraiQtdeProd(txt_codProd.Text, FilialProd_Ref, mNovaQtde, conection, transacao)
            ...
            Else
                ...
                _clBD.subtraiQtdeProd(txt_codProd.Text, FilialProd_Ref, CDbl(txt_qtde.Text), conection, transacao)
    ...
    Private Sub incluiRegistroPedidoTemporaria(ByVal conexao As NpgsqlConnection, ByVal transacao As NpgsqlTransaction)
        ...
        'nt_sit -- Verifica status do Pedido 1-Digitado , 2-Impresso, 3 - ECF , 4-NFe, 5-Cancelado
        nt_sit = 1
        nt_orca = mNumPedidoTemp

        _clBD.incPedido_Orca1Temporaria(nt_orca, nt_geno, nt_codig, nt_dtemis, nt_dtsai, nt_emiss, nt_cfop, _
                              nt_vend, nt_cid, nt_itens, nt_rota, nt_peso, nt_x, nt_y, nt_parc, _
                              nt_volum, nt_tipo2, nt_auto, nt_auto2, nt_cod1, nt_cod2, nt_cod3, _
                              nt_cod4, nt_cod5, nt_cod6, nt_cod7, nt_mapa, nt_sit, nt_tiposelecao, _
                              conexao, transacao)

        mIdOrca1ppTemp = _clBD.trazIdOrca1ppTemporaria(conexao, nt_orca, MdlUsuarioLogando._local)



    End Sub

    Private Sub inclueOrca4PedidoTemporaria(ByVal conexao As NpgsqlConnection, ByVal transacao As NpgsqlTransaction)
        ...
        tipo = "P" : nume = mNumPedidoTemp : tipo2 = cbo_forpgto.SelectedItem
        ...
        _clBD.incPedido_Orca4Temporaria(tipo, nume, tprod, aliss, vliss, vlser, basec, icms, bsub, icsub, tpro2, _
                              frete, segu, outros, ipi, tgeral, pgto, peso, desc, tipo2, MdlUsuarioLogando._local, conexao, transacao)

    End Sub

    Private Sub executaIncluiOrca1Orca4TblTemp()
        ...
        Try
            ...
            incluiRegistroPedidoTemporaria(conexao, transacao)
            inclueOrca4PedidoTemporaria(conexao, transacao)
    ...
    Private Sub incluiDtg_itensPedidoTemporaria(ByVal conexao As NpgsqlConnection, ByVal transacao As NpgsqlTransaction)
        ...
        Dim mdtemis As Date
        Dim mLinha, mGrupo, mrota As Integer
        ...
        For Each row As DataGridViewRow In Me.dtg_pedidoprotaentrega.Rows
            ...
            If Not row.IsNewRow Then
                ...
                _clBD.incPedido_Orca2Temporaria(Mid(cbo_local.SelectedItem, 1, 2), _numPedidoTemp, mCodpr, MUnd, mQtde, mPrvenda, malqdesc, _
                                       ...
                                       moutrasDesp, mvlicms, conexao, transacao)
    ...
    Private Sub addItemGrid()
        ...
        Try
            ...
            conection.ClearPool() : conection.Close() : conection = Nothing : mlinha = Nothing
    ...
    Private Sub incluiDtg_itens(ByVal conexao As NpgsqlConnection, ByVal transacao As NpgsqlTransaction)
        ...
        Dim mvlicms As Double
        Dim mdtemis As Date
        Dim mLinha, mGrupo, mrota, mmapa As Integer
            ...
            If verificaRegistroPedido() Then
                ...
                Else
                    ...
                    Try
                        ...
                        _clBD.alteraPedidoOrcamento_Orca1(_numOrcamento, Me.txt_pedido.Text, conexao, transacao)
    ...
    Private Sub btn_alterar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_alterar.Click
        ...
        Try
            If Not dtg_pedidoprotaentrega.CurrentRow.IsNewRow Then
                ...
                ValorUnitProd_Ref = dtg_pedidoprotaentrega.CurrentRow.Cells(23).Value
    ...
    Private Sub DeleteItemGrid()
        ...
        Try
            ...
            If Me.dtg_pedidoprotaentrega.Enabled = True Then
                ...
                conection.Open()
                transacao = conection.BeginTransaction
                _clBD.somaQtdeProd(codigoProduto, mLocal, qtdeProduto, conection, transacao)
    ...
    Private Sub preenchCamposRegistroPedido(ByVal numPedido As String)
        ...
        Try
            ...
            sqlPedido.Append("nt_cod6, nt_cod7, nt_mapa, nt_sit, nt_id ") '29 'cad.p_portad, cad.p_consumo 
            sqlPedido.Append("FROM " & MdlEmpresaUsu._esqEstab & ".tb_orca1 LEFT JOIN cadp001 cad ON ") '25
            sqlPedido.Append("cad.p_cod = nt_codig WHERE nt_orca = @nt_orca")
    ...
    Private Sub preenchItensPedido(ByVal numPedido As String)
        ...
        Try
            ...
            sqlPedido.Append("SELECT no_id, no_codpr, no_cdbarra, e_produt, no_und, no_qtde, no_pruvenda, no_prtot, ") '7
            ...
            sqlPedido.Append("no_outrasdesp, no_vlicms, no_prunit ") '24
            sqlPedido.Append("FROM " & MdlEmpresaUsu._esqEstab & ".tb_orca2 LEFT JOIN ")
            ...
            While drPedido.Read
                ...
                'Subtrai a quantidade do Estoque a quantidade requerida para o Pedido...
                mqtde = _clFuncoes.trazQtdeEstloja01(drPedido(1).ToString, drPedido(20).ToString, MdlConexaoBD.conectionPadrao)

                If mqtde > 0 Then

                    transacao = conexao.BeginTransaction
                    If drPedido(5) <= mqtde Then 'Se a quantidade do pedido for <= que a quantidade atual

                        _clBD.subtraiQtdeProd(drPedido(1).ToString, drPedido(20).ToString, drPedido(5), conexao, transacao)
                        ...
                        drPedido(18), drPedido(19), drPedido(20).ToString, drPedido(21), drPedido(22), drPedido(23), drPedido(24)}
                    ...
                    Else
                        _clBD.subtraiQtdeProd(drPedido(1).ToString, drPedido(20).ToString, mqtde, conexao, transacao)

                        Dim mlinha As String() = {drPedido(0), Mid(Me.cbo_local.SelectedItem, 1, 2), drPedido(1), drPedido(2), drPedido(3), drPedido(4), _
                        Format(mqtde, "###,##0.00"), Format(drPedido(6), "###,##0.00"), Format(Round((Round(mqtde * drPedido(6), 2) + drPedido(16)), 2), "###,##0.00"), _
                        drPedido(8), drPedido(9), drPedido(10), drPedido(11), _
                        drPedido(12), drPedido(13), drPedido(14), drPedido(15), drPedido(16), drPedido(17), _
                        drPedido(18), drPedido(19).ToString, drPedido(20).ToString, drPedido(21), drPedido(22), drPedido(23), drPedido(24)}
            ...
            drPedido.Close() : conexao.ClearPool() : conexao.Close() : conexao = Nothing : transacao = Nothing
    ...
    Private Sub Frm_OrcamentoCop_FormClosing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing

        If Me.dtg_pedidoprotaentrega.Rows.Count > _valorZERO Then

            Select Case MessageBox.Show("Deseja salvar esse pedido, para continuar mais tarde?", "METROSYS", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Information)

                Case Windows.Forms.DialogResult.Yes
                    'Faz nada, por que o pedido já está sendo gravado na tabela temporária na 
                    'hora de incluir os itens...
                ...
                Case Windows.Forms.DialogResult.No
                    ...
                    Dim transacao As NpgsqlTransaction
                    transacao = conection.BeginTransaction
                    cancelaPedido(Me.txt_pedido.Text, MdlUsuarioLogando._local, conection, transacao)
                    Try
    ...
    Private Sub cancelaPedido(ByVal numPedido As String, ByVal loja As String, _
        ...
        Try
            ...
            sqlPedido.Append("nt_vend, nt_cid, nt_itens, nt_rota, nt_peso, nt_x, nt_y, nt_parc, nt_cod1, ") '15
            sqlPedido.Append("nt_cod2, nt_volum, nt_tipo2, nt_auto, nt_auto2, nt_cod3, nt_cod4, nt_cod5, ") '23
            sqlPedido.Append("nt_cod6, nt_cod7, nt_mapa, nt_sit, cad.p_uf, nt_tiposelecao ") '29
        ...
        Try
            ...
            For Each row As DataGridViewRow In Me.dtg_pedidoprotaentrega.Rows
                ...
                If Not row.IsNewRow Then
                    ...
                    _clBD.somaQtdeProd(codigoProduto, mloja, qtdeProduto, conection, transacao)

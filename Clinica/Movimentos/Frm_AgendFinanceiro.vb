...
Public Class Frm_AgendFinanceiro
    ...
    ' *******************************************************************************
    ' ****  Vetores auxiliares de Gravação do Numero, vencimento e Valor do Np  **
    ' ** Não sera utilizado o array 0, somente a artir do 1
    Dim NumNp(21) As String
    Dim VlrNp(21) As Double
    Dim Vencto(21) As String
    '   *******************************************************************************

    Private nomeCliente, endCliente, cidadeCliente, ufCliente, bairroCliente As String
    ...
    Private Sub Frm_AgendFinanceiro_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Escape) Then Me.Close()
    End Sub

    Private Sub Frm_AgendFinanceiro_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
    ...
    Private Sub Frm_AgendFinanceiro_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load

        lbl_NomeSys.Text = Application.ProductName

        txt_cliente.Text = _Cliente.pPortad
        txt_subtotal.Text = Format(_clAgend1.a_valor, "###,##0.00")
        txt_total.Text = Format(_clAgend1.a_valor, "###,##0.00")
        cbo_tipo.SelectedIndex = 0
        Dim mId As Int64 = _clBD.trazProxIdFatd001(_Geno, MdlConexaoBD.conectionPadrao)
        'Dim maxId As Int64 = _clBD.trazMaxIdFatd001(_Geno, MdlConexaoBD.conectionPadrao)
        'If mId <= maxId Then
        '    _clBD.SetaIdFatd001(maxId, _Geno, MdlConexaoBD.conectionPadrao)
        '    mId = _clBD.trazProxIdFatd001(_Geno, MdlConexaoBD.conectionPadrao)
        'End If
        lbl_contrato.Text = String.Format("{0:D9}", mId)
        txt_entrada.Focus() : txt_entrada.SelectAll()
    ...
    Private Sub Recebe_vetoresAuxilia_ate_10()
        ' Vetores Auxiliares
        NumNp(0) = txt_num1.Text : VlrNp(0) = txt_valor1.Text : Vencto(0) = msk_vencto1.Text
        NumNp(1) = txt_num2.Text : VlrNp(1) = txt_valor2.Text : Vencto(1) = msk_vencto2.Text
        NumNp(2) = txt_num3.Text : VlrNp(2) = txt_valor3.Text : Vencto(2) = msk_vencto3.Text
        NumNp(3) = txt_num4.Text : VlrNp(3) = txt_valor4.Text : Vencto(3) = msk_vencto4.Text
        NumNp(4) = txt_num5.Text : VlrNp(4) = txt_valor5.Text : Vencto(4) = msk_vencto5.Text
        NumNp(5) = txt_num6.Text : VlrNp(5) = txt_valor6.Text : Vencto(5) = msk_vencto6.Text
        NumNp(6) = txt_num7.Text : VlrNp(6) = txt_valor7.Text : Vencto(6) = msk_vencto7.Text
        NumNp(7) = txt_num8.Text : VlrNp(7) = txt_valor8.Text : Vencto(7) = msk_vencto8.Text
        NumNp(8) = txt_num9.Text : VlrNp(8) = txt_valor9.Text : Vencto(8) = msk_vencto9.Text
        NumNp(9) = txt_num10.Text : VlrNp(9) = txt_valor10.Text : Vencto(9) = msk_vencto10.Text
    End Sub
    ...
    Private Sub Recebe_VetoresAuxiliar_ate_15()
        ...
        NumNp(10) = txt_num11.Text : VlrNp(10) = txt_valor11.Text : Vencto(10) = msk_vencto11.Text
        NumNp(11) = txt_num12.Text : VlrNp(11) = txt_valor12.Text : Vencto(11) = msk_vencto12.Text
        NumNp(12) = txt_num13.Text : VlrNp(12) = txt_valor13.Text : Vencto(12) = msk_vencto13.Text
        NumNp(13) = txt_num14.Text : VlrNp(13) = txt_valor14.Text : Vencto(13) = msk_vencto14.Text
        NumNp(14) = txt_num15.Text : VlrNp(14) = txt_valor15.Text : Vencto(14) = msk_vencto15.Text
    End Sub

    Function trazDataVenc(ByRef dataVencAtual As Date) As Date
        Dim mData As Date = dataVencAtual.AddDays(Mdias)
    ...
    Private Sub recebe_parcelas(ByVal ValorParc As Double, ByVal ValorUltima As Double, ByVal Parcelas As Integer)
        ...
        Select Case Parcelas
            Case 1
                ...
                Me.msk_vencto1.Text = Format(Mdata, "dd/MM/yyyy") 'Data_fixa(Mdata, Now.Day)
                ' Vetores Auxiliares
                NumNp(1) = txt_num1.Text : VlrNp(1) = txt_valor1.Text : Vencto(1) = msk_vencto1.Text
            Case 2
                Me.txt_valor1.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor2.Text = Format(Convert.ToDouble(ValorUltima), "##,##0.00")
                Me.msk_vencto1.Text = Format(Mdata, "dd/MM/yyyy") : Me.msk_vencto2.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                ' Numero do carne
            ...
            Case 3
                ...
                'Vencimento
                Me.msk_vencto1.Text = Data_fixa(Mdata, Now.Day) : Me.msk_vencto2.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto3.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                ' Numero do carne
            ...
            Case 4
                ...
                ' Vencimento
                Me.msk_vencto1.Text = Data_fixa(Mdata, Now.Day) : Me.msk_vencto2.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto3.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto4.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                ' Numero do carne
                Me.txt_num1.Text = lbl_contrato.Text + "A" : Me.txt_num2.Text = lbl_contrato.Text + "B" : Me.txt_num3.Text = lbl_contrato.Text + "C"
            ...
            Case 5
                ...
                Me.msk_vencto1.Text = Data_fixa(Mdata, Now.Day) : Me.msk_vencto2.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto3.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto4.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto5.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                ' Numero do carne
                Me.txt_num1.Text = lbl_contrato.Text + "A" : Me.txt_num2.Text = lbl_contrato.Text + "B" : Me.txt_num3.Text = lbl_contrato.Text + "C"
            ...
            Case 6
                ...
                Me.msk_vencto1.Text = Data_fixa(Mdata, Now.Day) : Me.msk_vencto2.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto3.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto4.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto5.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto6.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                ' Numero do carne
                Me.txt_num1.Text = lbl_contrato.Text + "A" : Me.txt_num2.Text = lbl_contrato.Text + "B" : Me.txt_num3.Text = lbl_contrato.Text + "C"
            ...
            Case 7
                ...
                ' Vencimentos
                Me.msk_vencto1.Text = Data_fixa(Mdata, Now.Day) : Me.msk_vencto2.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto3.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto4.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto5.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto6.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto7.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                ' Numero do carne
                Me.txt_num1.Text = lbl_contrato.Text + "A" : Me.txt_num2.Text = lbl_contrato.Text + "B" : Me.txt_num3.Text = lbl_contrato.Text + "C"
                Me.txt_num4.Text = lbl_contrato.Text + "D" : Me.txt_num5.Text = lbl_contrato.Text + "E" : Me.txt_num6.Text = lbl_contrato.Text + "F"
            ...
            Case 8
                ...
                Me.msk_vencto4.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto5.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto6.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto7.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto8.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                ' Numero do carne
                Me.txt_num1.Text = lbl_contrato.Text + "A" : Me.txt_num2.Text = lbl_contrato.Text + "B" : Me.txt_num3.Text = lbl_contrato.Text + "C"
                Me.txt_num4.Text = lbl_contrato.Text + "D" : Me.txt_num5.Text = lbl_contrato.Text + "E" : Me.txt_num6.Text = lbl_contrato.Text + "F"
            ...
            Case 9
                ...
                Me.msk_vencto4.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto5.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto6.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto7.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto8.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto9.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                ' Numero do carne
                Me.txt_num1.Text = lbl_contrato.Text + "A" : Me.txt_num2.Text = lbl_contrato.Text + "B" : Me.txt_num3.Text = lbl_contrato.Text + "C"
                Me.txt_num4.Text = lbl_contrato.Text + "D" : Me.txt_num5.Text = lbl_contrato.Text + "E" : Me.txt_num6.Text = lbl_contrato.Text + "F"
            ...
            Case 10
                ...
                Me.msk_vencto3.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto4.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto5.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto6.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto7.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto8.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Me.msk_vencto9.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy") : Me.msk_vencto10.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                ' Numero do carne
                Me.txt_num1.Text = lbl_contrato.Text + "A" : Me.txt_num2.Text = lbl_contrato.Text + "B" : Me.txt_num3.Text = lbl_contrato.Text + "C"
                Me.txt_num4.Text = lbl_contrato.Text + "D" : Me.txt_num5.Text = lbl_contrato.Text + "E" : Me.txt_num6.Text = lbl_contrato.Text + "F"
                Me.txt_num7.Text = lbl_contrato.Text + "G" : Me.txt_num8.Text = lbl_contrato.Text + "H" : Me.txt_num9.Text = lbl_contrato.Text + "I"
            ...
            Case 16
                ...
                NumNp(15) = txt_num16.Text : VlrNp(15) = txt_valor16.Text : Vencto(15) = msk_vencto16.Text
            Case 17
                Recebe_Parcela_ate_10(ValorParc)
                Me.txt_valor11.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor12.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00")
                ...
                Recebe_VetoresAuxiliar_ate_15()
                NumNp(15) = txt_num16.Text : VlrNp(15) = txt_valor16.Text : Vencto(15) = msk_vencto16.Text
                NumNp(16) = txt_num17.Text : VlrNp(16) = txt_valor17.Text : Vencto(16) = msk_vencto17.Text
            Case 18
                Recebe_Parcela_ate_10(ValorParc)
                Me.txt_valor11.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor12.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00")
                Me.txt_valor13.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor14.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor15.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00")
                ...
                NumNp(15) = txt_num16.Text : VlrNp(15) = txt_valor16.Text : Vencto(15) = msk_vencto16.Text
                NumNp(16) = txt_num17.Text : VlrNp(16) = txt_valor17.Text : Vencto(16) = msk_vencto17.Text
                NumNp(17) = txt_num18.Text : VlrNp(17) = txt_valor18.Text : Vencto(17) = msk_vencto18.Text
            Case 19
                Recebe_Parcela_ate_10(ValorParc)
                Me.txt_valor11.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor12.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00")
                Me.txt_valor13.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor14.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor15.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00")
                ...
                NumNp(16) = txt_num17.Text : VlrNp(16) = txt_valor17.Text : Vencto(16) = msk_vencto17.Text
                NumNp(17) = txt_num18.Text : VlrNp(17) = txt_valor18.Text : Vencto(17) = msk_vencto18.Text
                NumNp(18) = txt_num19.Text : VlrNp(18) = txt_valor19.Text : Vencto(18) = msk_vencto19.Text
            Case 20
                Recebe_Parcela_ate_10(ValorParc)
                Me.txt_valor11.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor12.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00")
                Me.txt_valor13.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor14.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00") : Me.txt_valor15.Text = Format(Convert.ToDouble(ValorParc), "##,##0.00")
                ...
                NumNp(17) = txt_num18.Text : VlrNp(17) = txt_valor18.Text : Vencto(17) = msk_vencto18.Text
                NumNp(18) = txt_num19.Text : VlrNp(18) = txt_valor19.Text : Vencto(18) = msk_vencto19.Text
                NumNp(19) = txt_num20.Text : VlrNp(19) = txt_valor20.Text : Vencto(19) = msk_vencto20.Text
        End Select
    End Sub
    ...
    Private Sub Recebe_Venctos(qtdeParcelas As Int16)

        If qtdeParcelas > 20 Then qtdeParcelas = 20
        For indx As Int16 = 1 To qtdeParcelas

            Select Case indx
                Case 1
                    Me.msk_vencto1.Text = Data_fixa(Mdata, Now.Day)
                Case 2
                    Me.msk_vencto2.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 3
                    Me.msk_vencto3.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 4
                    Me.msk_vencto4.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 5
                    Me.msk_vencto5.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 6
                    Me.msk_vencto6.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 7
                    Me.msk_vencto7.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 8
                    Me.msk_vencto8.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 9
                    Me.msk_vencto9.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 10
                    Me.msk_vencto10.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 11
                    Me.msk_vencto11.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 12
                    Me.msk_vencto12.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 13
                    Me.msk_vencto13.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 14
                    Me.msk_vencto14.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 15
                    Me.msk_vencto15.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 16
                    Me.msk_vencto16.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 17
                    Me.msk_vencto17.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 18
                    Me.msk_vencto18.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 19
                    Me.msk_vencto19.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
                Case 20
                    Me.msk_vencto20.Text = Format(trazDataVenc(Mdata), "dd/MM/yyyy")
            End Select
    ...
    Private Sub Recebe_NumeroNp_ate_10()
        Me.txt_num1.Text = lbl_contrato.Text + "A" : Me.txt_num2.Text = lbl_contrato.Text + "B" : Me.txt_num3.Text = lbl_contrato.Text + "C"
        Me.txt_num4.Text = lbl_contrato.Text + "D" : Me.txt_num5.Text = lbl_contrato.Text + "E" : Me.txt_num6.Text = lbl_contrato.Text + "F"
        Me.txt_num7.Text = lbl_contrato.Text + "G" : Me.txt_num8.Text = lbl_contrato.Text + "H" : Me.txt_num9.Text = lbl_contrato.Text + "I"
        Me.txt_num10.Text = lbl_contrato.Text + "J"
    ...
    Private Function Data_fixa(ByVal C_DataVenc As Date, ByVal C_DiaFixo As Integer) As Date
        ...
        Try
            ...
            Try
                Return Convert.ChangeType(XDataFixa, GetType(Date))
            Catch ex As Exception

                mDtAux1 = Format(DateSerial(C_DataVenc.Year, C_DataVenc.Month + 1, 0), "dd/MM/yyyy")
                mDiaAdd = 1
                C_DataVenc = Convert.ChangeType(mDtAux1, GetType(Date)).AddDays(mDiaAdd)

                Return Convert.ChangeType(C_DataVenc, GetType(Date))
    ...
    Private Sub btn_gravar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_gravar.Click
        ...
        Dim x, t As Integer
        Dim hora As String = Mid(TimeString, 1, 5)
        ...
        If txt_num1.Text.Equals("") Then Return
        Try
            SetaVencimentos()
            SetaValorNP()

            If CDbl(txt_entrada.Text) > 0 Then
                Xfuncao.incEntradaFatd001(conexao, transacao, _Geno, CDbl(txt_entrada.Text), Convert.ToInt64(lbl_contrato.Text), Date.Now)
            End If

            For x = 1 To (Convert.ToInt16(txt_qtde.Text))
                Xfuncao.IncNP_a_Receber(_clAgend1, CInt(txt_qtde.Text), _Geno.pCodig, _Cliente.pCod, _mTipo, lbl_contrato.Text, lbl_contrato.Text, "", 0.0, NumNp(x - 1), Date.Now, _
                      DateValue(Vencto(x - 1)), CDbl(VlrNp(x - 1)), "00", Now, 0.0, 0.0, "000", "", "", 0.0, 0.0, "", "", "", "N", False, _Geno.pCodig, 0, "", "", _
                      "", "", "", "", 0.0, 0.0, 0.0, CDbl(txt_taxaCobranca.Text), CDbl(txt_valorTaxa.Text), _Dentista.Nome, hora, conexao, transacao)
            Next
            Me.lbl_mensagem.Text = "Duplicatas Incluidas com Sucesso !"
...
#Region "   * *  Tratamentos para impressão  * *   "
    ...
    Private Sub executaEspelhoNF_R(ByVal unidadePC As String, ByVal arqSaida As String)
        ...
        Dim mArqTemp As String = "\wged\relatorios\TEMPnp.TMP"
    ...
    Private Sub rptGravaTotaisNF(ByVal sender As Object, ByVal Relatorio As System.Drawing.Printing.PrintPageEventArgs)
        ...
        mQtdeParcelas = Format(CInt(txt_qtde.Text), "00")
        ...
        If (Me.txt_num1.Text.Equals("") = False) AndAlso (mParcela1OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Mid(cidadeCliente, 1, 30) & ", " & ufCliente, mFonte, Brushes.Black, margemEsq - 335, (95 + mLinhaAtualLetras), New StringFormat())
            Relatorio.Graphics.DrawString(bairroCliente, mFonte, Brushes.Black, margemEsq - 335, (110 + mLinhaAtualLetras), New StringFormat())
            Relatorio.Graphics.DrawString(endCliente, mFonte, Brushes.Black, margemEsq - 335, (67 + mLinhaAtualLetras), New StringFormat())
            ...
            Relatorio.Graphics.DrawString(Me.txt_num1.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num2.Text.Equals("") = False) AndAlso (mParcela2OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num2.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num3.Text.Equals("") = False) AndAlso (mParcela3OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num3.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num4.Text.Equals("") = False) AndAlso (mParcela4OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num4.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num5.Text.Equals("") = False) AndAlso (mParcela5OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num5.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num6.Text.Equals("") = False) AndAlso (mParcela6OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num6.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num7.Text.Equals("") = False) AndAlso (mParcela7OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num7.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num8.Text.Equals("") = False) AndAlso (mParcela8OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num8.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num9.Text.Equals("") = False) AndAlso (mParcela9OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num9.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num10.Text.Equals("") = False) AndAlso (mParcela10OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num10.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num11.Text.Equals("") = False) AndAlso (mParcela11OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num11.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num12.Text.Equals("") = False) AndAlso (mParcela12OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num12.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num13.Text.Equals("") = False) AndAlso (mParcela13OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num13.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num14.Text.Equals("") = False) AndAlso (mParcela14OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num14.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num15.Text.Equals("") = False) AndAlso (mParcela15OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num15.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num16.Text.Equals("") = False) AndAlso (mParcela16OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num16.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num17.Text.Equals("") = False) AndAlso (mParcela17OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num17.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num18.Text.Equals("") = False) AndAlso (mParcela18OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num18.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num19.Text.Equals("") = False) AndAlso (mParcela19OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num19.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
        ...
        If (Me.txt_num20.Text.Equals("") = False) AndAlso (mParcela20OK = False) Then
            ...
            Relatorio.Graphics.DrawString(Me.txt_num20.Text & "/" & mQtdeParcelas, mFonte, Brushes.Black, margemEsq - 130, (175 + mLinhaAtualLetras), New StringFormat())
...
#End Region
    ...
    Private Sub cbo_tipo_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbo_tipo.SelectedIndexChanged
        ...
        Try
            _mTipo = Mid(cbo_tipo.SelectedItem.ToString, 1, 2)
        Catch ex As Exception
        End Try

        Select Case cbo_tipo.SelectedIndex
            Case 2 'Cartão
                txt_qtde.Text = "1" : txt_qtde.ReadOnly = False
                txt_prazo.Text = "30" : txt_prazo.ReadOnly = False

                txt_taxaCobranca.Text = "0,00" : txt_valorTaxa.Text = "0,00"
                txt_taxaCobranca.ReadOnly = False
            ...
            Case Else
                ...
                txt_taxaCobranca.Text = "0,00" : txt_valorTaxa.Text = "0,00"
                txt_taxaCobranca.ReadOnly = True
        End Select

    End Sub

    Private Sub txt_prazo_Leave(sender As Object, e As EventArgs) Handles txt_prazo.Leave

        lbl_mensagem.Text = ""
        If Me.txt_prazo.Text.Equals("") Then
            Me.txt_prazo.Text = "0"
            If cbo_tipo.SelectedIndex > 0 Then Me.txt_prazo.Text = "1"
        End If

        If IsNumeric(Me.txt_prazo.Text) Then

            If cbo_tipo.SelectedIndex > 0 Then

                If CDec(Me.txt_prazo.Text) <= _valorZERO Then
                    lbl_mensagem.Text = "Prazo de Dias deve ser maior que ZERO !"
                    Return
    ...
    Private Sub txt_prazo_TextChanged(sender As Object, e As EventArgs) Handles txt_prazo.TextChanged

        Try
            Mdias = txt_prazo.Text
        Catch ex As Exception
            Mdias = 0
        End Try
    ...
    Sub GeraParcelas()
        ...
        vlParcelas = Round(CDbl(Me.txt_total.Text) / Convert.ToInt16(Me.txt_qtde.Text), 2) * 100
        vlParcelas = (vlParcelas / 100)
        vldiferenca = vlParcelas * (Convert.ToInt16(Me.txt_qtde.Text) - 1)
        vlultima = CDbl(Me.txt_total.Text) - vldiferenca

        If Convert.ToInt16(txt_qtde.Text) = 1 Then
            Try
                Me.txt_valor1.Text = Convert.ToInt32(CDbl(Me.txt_total.Text))
                Me.txt_valor1.Text = Format(Convert.ToDouble(Me.txt_total.Text), "##,##0.00")
                Mdata = Now.AddDays(Mdias)
                Me.msk_vencto1.Text = Data_fixa(Mdata, Now.Day)
    ...
    Sub SetaVencimentos()
        ...
        For parcela As Int16 = 1 To CInt(txt_qtde.Text)

            Select Case parcela
                Case 1
                    Vencto(0) = msk_vencto1.Text
                Case 2
                    Vencto(1) = msk_vencto2.Text
                Case 3
                    Vencto(2) = msk_vencto3.Text
                Case 4
                    Vencto(3) = msk_vencto4.Text
                Case 5
                    Vencto(4) = msk_vencto5.Text
                Case 6
                    Vencto(5) = msk_vencto6.Text
                Case 7
                    Vencto(6) = msk_vencto7.Text
                Case 8
                    Vencto(7) = msk_vencto8.Text
                Case 9
                    Vencto(8) = msk_vencto9.Text
                Case 10
                    Vencto(9) = msk_vencto10.Text
                Case 11
                    Vencto(10) = msk_vencto11.Text
                Case 12
                    Vencto(11) = msk_vencto12.Text
                Case 13
                    Vencto(12) = msk_vencto13.Text
                Case 14
                    Vencto(13) = msk_vencto14.Text
                Case 15
                    Vencto(14) = msk_vencto15.Text
                Case 16
                    Vencto(15) = msk_vencto16.Text
                Case 17
                    Vencto(16) = msk_vencto17.Text
                Case 18
                    Vencto(17) = msk_vencto18.Text
                Case 19
                    Vencto(18) = msk_vencto19.Text
                Case 20
                    Vencto(19) = msk_vencto20.Text
            End Select
        Next
    ...
    Sub SetaValorNP()
        ...
        For parcela As Int16 = 1 To CInt(txt_qtde.Text)
            ...
            Select Case parcela
                Case 1
                    VlrNp(0) = txt_valor1.Text
                Case 2
                    VlrNp(1) = txt_valor2.Text
                Case 3
                    VlrNp(2) = txt_valor3.Text
                Case 4
                    VlrNp(3) = txt_valor4.Text
                Case 5
                    VlrNp(4) = txt_valor5.Text
                Case 6
                    VlrNp(5) = txt_valor6.Text
                Case 7
                    VlrNp(6) = txt_valor7.Text
                Case 8
                    VlrNp(7) = txt_valor8.Text
                Case 9
                    VlrNp(8) = txt_valor9.Text
                Case 10
                    VlrNp(9) = txt_valor10.Text
                Case 11
                    VlrNp(10) = txt_valor11.Text
                Case 12
                    VlrNp(11) = txt_valor12.Text
                Case 13
                    VlrNp(12) = txt_valor13.Text
                Case 14
                    VlrNp(13) = txt_valor14.Text
                Case 15
                    VlrNp(14) = txt_valor15.Text
                Case 16
                    VlrNp(15) = txt_valor16.Text
                Case 17
                    VlrNp(16) = txt_valor17.Text
                Case 18
                    VlrNp(17) = txt_valor18.Text
                Case 19
                    VlrNp(18) = txt_valor19.Text
                Case 20
                    VlrNp(19) = txt_valor20.Text
            End Select
        Next

    End Sub

    Private Sub txt_taxaCobranca_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txt_taxaCobranca.KeyPress, txt_valorTaxa.KeyPress, txt_entrada.KeyPress, txt_valor1.KeyPress, txt_valor2.KeyPress, txt_valor3.KeyPress, txt_valor4.KeyPress, txt_valor5.KeyPress, txt_valor6.KeyPress, txt_valor7.KeyPress, txt_valor8.KeyPress, txt_valor9.KeyPress, txt_valor10.KeyPress
        'permite só numeros com vírgula
        If _clFunc.SoNumerosVirgula(CShort(Asc(e.KeyChar))) = _valorZERO Then e.Handled = True
    End Sub

    Private Sub txt_taxaCobranca_Leave(sender As Object, e As EventArgs) Handles txt_taxaCobranca.Leave

        lbl_mensagem.Text = ""
        If Me.txt_taxaCobranca.Text.Equals("") Then Me.txt_taxaCobranca.Text = Format(0.0, "###,##0.00")
        If IsNumeric(Me.txt_taxaCobranca.Text) Then
            If CDec(Me.txt_taxaCobranca.Text) < _valorZERO Then
                lbl_mensagem.Text = "Taxa de Cobrança deve ser Maior ou Igual a ZERO !"
                Me.txt_taxaCobranca.Focus() : Me.txt_taxaCobranca.SelectAll()
                Return

            End If
            Me.txt_taxaCobranca.Text = Format(CDec(Me.txt_taxaCobranca.Text), "###,##0.00")
            Try
                Me.txt_valorTaxa.Text = Format(Round((CDbl(Me.txt_subtotal.Text) * CDbl(Me.txt_taxaCobranca.Text)) / 100, 2), "###,##0.00")
            Catch ex As Exception
                Me.txt_valorTaxa.Text = Format(0, "###,##0.00")
            End Try
            Me.txt_total.Text = Format(Round(CDbl(Me.txt_subtotal.Text) - (CDbl(Me.txt_valorTaxa.Text) + CDbl(txt_entrada.Text)), 2), "###,##0.00")

        End If

    End Sub

    Private Sub txt_entrada_Leave(sender As Object, e As EventArgs) Handles txt_entrada.Leave
        ...
        If IsNumeric(Me.txt_entrada.Text) Then
            If CDec(Me.txt_entrada.Text) < _valorZERO Then
                ...
                Me.txt_entrada.Focus() : Me.txt_entrada.SelectAll()
                Return

...
Public Class Frm_buscaFornecedor
    ...
    Private Sub setValores()
        ...
        Try
            _formRequest._frmREf.mCliente.pIsento = Me.Dg_particpante.CurrentRow.Cells(10).Value
            _formRequest._frmREf.mCliente.pConsumo = Me.Dg_particpante.CurrentRow.Cells(11).Value.ToString
            _formRequest._frmREf.mCliente.pCarac = Me.Dg_particpante.CurrentRow.Cells(12).Value.ToString
    ...
    Private Sub preencheDgrd_Participante(ByVal ultimChar As String)
        ...
        If oConnBDGENOV.State = ConnectionState.Open Then
            Dim codigo, nome, cpf_cnpj, inscricao, UF, endereco, cidade, cep, fone, consumo, carac As String
            Dim isento As Boolean

            Try
                SqlParticipante.Append("SELECT p_cod, p_portad, p_cgc, p_cpf, p_insc, p_uf, p_end, p_cid, ") ' 7
                SqlParticipante.Append("p_cep, p_fone, p_consumo, p_isento, p_carac FROM cadp001 WHERE p_inativo = FALSE ") ' 12

...
Public Class Frm_GeraOrcamento
    ...
    Private Sub preecheDtgPedidos2()
        ...
        If MdlUsuarioLogando._usuarioPrivilegio Then 'verifica se usuário tem privilégio...
            ...
            Sqlcomm.Append("FROM " & MdlEmpresaUsu._esqEstab & ".tb_orca1 n1, cadp001 cad, " & MdlEmpresaUsu._esqEstab & ".tb_orca4 n4 where n1.nt_codig=cad.p_cod and n4.n4_nume=n1.nt_orca order by n1.nt_dtemis DESC ")
        Else

            Sqlcomm.Append("Select n1.nt_id, n1.nt_geno, n1.nt_orca AS ""Pedido"", n1.nt_dtemis AS ""Emissão"",n1.nt_codig AS ""Codigo"",cad.p_portad AS ""Cliente"",cad.p_cid AS ""Cidade"",cad.p_uf AS ""UF"" ,n4.n4_tgeral AS ""Valor R$"", n1.nt_vend AS ""Vendedor"" ") '12
            Sqlcomm.Append("FROM " & MdlEmpresaUsu._esqEstab & ".tb_orca1 n1, cadp001 cad, " & MdlEmpresaUsu._esqEstab & ".tb_orca4 n4 where n1.nt_vend = '" & MdlUsuarioLogando._codvendedor & "' AND n1.nt_codig=cad.p_cod and n4.n4_nume=n1.nt_orca order by n1.nt_dtemis DESC ")
        End If
        'Sqlcomm.Append("desc limit 34")
    ...
    Private Sub preecheDtgPedidosPesquisa(ByVal pesquisa As String, ByVal pesquisa2 As String)
        ...
        End If
        Sqlcomm.Append("order by n1.nt_dtemis DESC")
    ...
    Private Sub executaF6()
        ...
        If (Me.dtg_pedidos.Rows.Count > _valorZERO) AndAlso (Me.dtg_pedidos.SelectedCells.Count > 0) Then
            ...
            executaRelatorioOrcamento("", "\wged\relatorios\consultaOrcamento.txt")
        End If
    ...
    Private Sub executaRelatorio2(ByVal s As Cl_EscreveArquivo, ByVal loja As String, ByVal numeroOrcamento As String, ByVal dtEmissao As String, ByVal codClient As String, ByVal nomeClient As String, ByVal condicao As String, ByVal codVendedor As String, ByVal idOrca1 As Int32, ByRef shouldReturn As Boolean)
    ...
    Private Sub executaRelatorioOrcamento(ByVal unidadePC As String, ByVal arqSaida As String)
        ...
        Dim mArqTemp As String = "\wged\tmp\TEMPconsultaOrca.TMP"
    ...
    Private Sub executaEspelhoPedido2(ByVal unidadePC As String, ByVal arqSaida As String)
        ...
        'Grava informações da(s) Nota(s) no arquivo de saida...
        Dim mArqTemp1 As String = "\wged\tmp\TEMPconsultaPed1.TMP"
        Dim mArqTemp2 As String = "\wged\tmp\TEMPconsultaPed2.TMP"
        Dim mArqTemp3 As String = "\wged\tmp\TEMPconsultaPed3.TMP"
    ...
    Private Sub GravCabecalhoLoja2(ByVal s As Cl_EscreveArquivo, ByVal loja As String, ByRef shouldReturn As Boolean)
    ...
    Private Sub GravCabecalhoCliente2(ByVal s As Cl_EscreveArquivo, ByVal codCliente As String, _
        ...
        Try
            ...
            strLinha += "|" & _clFuncoes.Exibe_StrEsquerda("Emis:" & Format(Convert.ChangeType(dtEmiss, GetType(Date)), "dd/MM/yyyy"), 15) & "|"
            s.WriteLine(_clFuncoes.Exibe_cabecalho(strLinha, 4, 80))
    ...
    Private Sub GravItensArq2(ByVal s As Cl_EscreveArquivo, ByVal numPedido As String, ByVal idOrca1 As Int32, _
        ...
        If cbo_opcoes.SelectedIndex >= _valorZERO Then
            ...
            Select Case cbo_opcoes.SelectedIndex
                ...
                Case 1 'Data
                    ...
                    Try
                        msk_pesquisa.Text = Format(Convert.ChangeType(msk_pesquisa.Text, GetType(Date)), "dd/MM/yyyy")
                        msk_periodoFinal.Text = Format(Convert.ChangeType(msk_periodoFinal.Text, GetType(Date)), "dd/MM/yyyy")
                    Catch ex As Exception
                    End Try
                ...
                Select Case cbo_opcoes.SelectedIndex
                    ...
                    Case 1 'Data
                        ...
                        If IsDate(msk_pesquisa.Text) AndAlso IsDate(msk_periodoFinal.Text) Then
                            ...
                            If Convert.ChangeType(msk_periodoFinal.Text, GetType(Date)) < Convert.ChangeType(msk_pesquisa.Text, GetType(Date)) Then

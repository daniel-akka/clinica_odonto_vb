...
Public Class Frm_NFEOutras
    ...
    Public _NFeS As New Cl_NFeSaida
...
#End Region
    ...
    Private Sub Frm_NFEOutras_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing

        If dtg_itensNFe.Rows.Count > 0 Then

            If MessageBox.Show("Ja existe itens para esta NFe! Deseja realmene Sair?", "Metrosys", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = _
    ...
    Private Sub devolveQtdeEstloja01()
        ...
        Select Case cbo_tiponfe.SelectedIndex

            Case 0 'Venda ou Bonificação (Acrescenta Estoque)

                Dim conection As New NpgsqlConnection(MdlConexaoBD.conectionPadrao)
                Dim transacao As NpgsqlTransaction
                Dim codigoProduto As String = ""
                Dim qtdeProduto As Double = 0
                Dim mloja As String = _clXml.codLoja
    ...
    Private Sub Frm_NFEOutras_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        cbo_estabelecimento.SelectedIndex = _clFuncoes.trazIndexCboLoja2dig(MdlEmpresaUsu._codigo.Substring(MdlEmpresaUsu._codigo.Length - 2, 2), cbo_estabelecimento)
...
#Region "   *** Funções e Procedimentos Auxiliares ***   "
    ...
    Private Function validaCamposItem() As Boolean
        ...
        Select Case _clXml._cst
            Case "30", "41", "60" '          ! Não informar os ICMS(Normal e Substituto) !
...
#Region "   *** Tratamento da NF-e ***   "
    ...
    Private Sub incluiRegistroNFe(ByVal conexao As NpgsqlConnection, ByVal transacao As NpgsqlTransaction, ByRef ok As Boolean)
        ...
        For Each row As DataGridViewRow In dtg_itensNFe.Rows
            ...
            If row.IsNewRow = False Then
                ...
                _NFeS.pNt2cc.pNc_reduz = row.Cells(28).Value
    ...
    Private Function buscaArquivosXML() As Boolean
        ...
        If _clXml.strXmlRetorno.Equals("") Then 'Se retornou nada...
            ...
            frmMsgRtbox.rtb_mensagem.Text = _clXml.strArqErroRetorno
        ...
        Else
            ...
            If _clXml.strXmlLoteRecebido.Equals("") = False Then ' se ele vinher alguma coisa
                ...
                Try
                    ...
                    dtg_itensNFe.Rows.Clear() : dtg_itensNFe.Refresh()
            ...
            Else
                ...
                frmMsgRtbox.rtb_mensagem.Text = _clXml.strXmlLoteRecebido
                Me.btn_finaliza.Enabled = False
                frmMsgRtbox.ShowDialog()
...
#End Region
    ...
    Private Sub cbo_nfeCfop_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbo_nfeCfop.Leave
        ...
        If cbo_nfeCfop.SelectedIndex >= 0 Then _clXml.mNFe_Cfop = cbo_nfeCfop.Text.Substring(0, 5)
        Try

            If Not cadp001.pUf.Equals("") Then

                If cadp001.pUf = geno001.pUf Then
                    If Mid(_clXml.mNFe_Cfop, 1, 1) <> "5" AndAlso Mid(_clXml.mNFe_Cfop, 1, 1) <> "1" Then
    ...
    Private Sub txt_basecalc_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_basecalc.GotFocus
        ...
        Try
            If _clXml._cst.Equals("00") OrElse _clXml._cst.Equals("10") Then 'txt_cfop.Text.Substring(txt_cfop.Text.Length - 3, 3).Equals("102")
    ...
    Private Sub btn_inclui_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_inclui.Click
        ...
        If validaCamposItem() OrElse _mContErroCST > 0 Then

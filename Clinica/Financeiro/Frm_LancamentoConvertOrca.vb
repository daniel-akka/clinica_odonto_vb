...
Public Class Frm_LancamentoConvertOrca
    ...
    Public _Orcamento As New Cl_CxOrcamento
    ...
    Public Shared _frmREf As New Frm_LancamentoConvertOrca

    Private Sub Frm_LancamentoConvertOrca_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        If (e.KeyCode = Keys.Escape) Then Me.Close()
    End Sub

    Private Sub Frm_LancamentoConvertOrca_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
    ...
    Private Sub Frm_LancamentoConvertOrca_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Me._idLancamento > 0 Then
            _Orcamento = _Orcamento.DAO.trazCX_DiarioOrca(Me._idLancamento, _Geno, MdlConexaoBD.conectionPadrao)
            If _Orcamento.oc_id < 1 Then
                MsgBox("Orçamento """ & Me._idLancamento & """ não encontrado na Tabela!", MsgBoxStyle.Critical)
                Me.Close()
            End If
    ...
    Sub SetaCamposLancamento()

        cbo_tipo.SelectedIndex = 1
        msk_data.Value = _Orcamento.oc_data
        txt_grupo.Text = "500"
        cbo_descricao.Text = _Orcamento.oc_descricao
        txt_valor.Text = Format(_Orcamento.oc_valor, "###,##0.00")
        txt_comiss.Text = Format(0.0, "###,##0.00")
        txt_hora.Text = _Orcamento.oc_hora

        If _Orcamento.oc_doutor.Equals("") = False Then
            _DoutorDAO.trazDoutorLojaNome(_Orcamento.oc_doutor, _Geno, _Doutor)
            cbo_doutores.SelectedIndex = _clFuncoes.trazIndexComboBox(_Doutor.Nome, _Doutor.Nome.Length, cbo_doutores)
        End If

        If _Orcamento.oc_codcli.Equals("") = False Then
            _clFuncoes.trazCadp001(_Orcamento.oc_codcli, _Cliente)
            txt_codPart.Text = _Cliente.pCod : txt_nomePart.Text = _Cliente.pPortad
        End If

        If _Orcamento.oc_protetico.Equals("") = False Then
            _ProteticoDAO.trazDoutorLojaNome(_Orcamento.oc_protetico, _Geno, _Protetico)
            cbo_protetico.SelectedIndex = _clFuncoes.trazIndexComboBox(_Protetico.Nome, _Protetico.Nome.Length, cbo_protetico)
        End If

        cbo_tpAtendimento.SelectedIndex = 0
        chk_recebidoAnteriormente.Checked = False
    ...
    Function ValidaValores() As Boolean
        ...
        Try
            ...
            If chk_recebidoAnteriormente.Checked = False Then
                If CDbl(Me.txt_total.Text) <= 0 Then
    ...
    Private Sub btn_incluir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_incluir.Click
        ...
        Else
            Try
                ...
                _CaixaDiario.cx_orcamento = True
                ...
                _Orcamento.DAO.delCX_Diario(_Orcamento, _Geno, MdlConexaoBD.conectionPadrao, False)

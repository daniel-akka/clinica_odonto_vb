...
Public Class Frm_cadMetro
    ...
    Private Sub Frm_cadGeno_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        cbo_blBancos = _clFuncoes.PreenchComboBancos(cbo_blBancos, MdlConexaoBD.conectionPadrao)
    ...
    Private Sub trazGenoSelecionado()
        ...
        Try
            ...
            While drGeno.Read
                ...
                _clGeno.pSn = drGeno(26) : _clGeno.pAtivEmpresa = drGeno(27)
                _clGeno.pBlBancoPadrao = drGeno(28).ToString : _clGeno.pBlOptPadrao = drGeno(29)
                ...
                cbo_ativEmpresa.SelectedIndex = _clFuncoes.trazIndexCboAtivEmpresa(_clGeno.pAtivEmpresa, cbo_ativEmpresa)
                cbo_blBancos.SelectedIndex = _clFuncoes.trazIndexCboBancos(_clGeno.pBlBancoPadrao, cbo_blBancos)
                cbo_optBoleto.SelectedIndex = _clFuncoes.trazIndexCboBlOptPadrao(_clGeno.pBlOptPadrao, cbo_optBoleto)
    ...
    Private Sub trazGenpSelecionado()
        ...
        Try
            ...
            sqlGenp.Append("gp_senhaftppalm, gp_pauta, gp_descontonfe, gp_descontopedido, gp_pastasgbd, gp_logomarcapath ") '62
            ...
            If drGenp.HasRows = False Then 'Se NÃO existir as configurações da empresa na tabela Genp001
                ...
                chk_descontopedido.Checked = _clGenp.descontopedido
            ...
            While drGenp.Read
                ...
                Try
                    _clGenp.pData = Convert.ChangeType(drGenp(3), GetType(Date))
                Catch ex As Exception
                ...
                _clGenp.descontopedido = drGenp(60)
                ...
                _clGenp.imagemCarne = drGenp(44).ToString : _clGenp.pastaSgbd = drGenp(61).ToString
                _clGenp.logomarcapath = drGenp(62).ToString
                ...
                chk_pauta.Checked = _clGenp.pauta : chk_descontonfe.Checked = _clGenp.descontonfe
                txt_geno.Text = _clGenp.pGeno : chk_descontopedido.Checked = _clGenp.descontopedido
                ...
                txt_ImagemCarne.Text = _clGenp.imagemCarne : txt_pastaSgbd.Text = _clGenp.pastaSgbd
                txt_pathLogoMarca.Text = _clGenp.logomarcapath
    ...
    Private Sub salvaGenoIncluindo(ByRef conection As NpgsqlConnection, ByRef transacao As NpgsqlTransaction)
        ...
        Try
            _clGeno.pAtivEmpresa = Mid(cbo_ativEmpresa.SelectedItem, 1, 1)
        Catch ex As Exception
        End Try

        Try
            _clGeno.pBlBancoPadrao = Mid(cbo_blBancos.SelectedItem, 1, 3)
        Catch ex As Exception
        End Try

        Try
            _clGeno.pBlOptPadrao = Mid(cbo_optBoleto.SelectedItem, 1, 1)
        Catch ex As Exception
        End Try
    ...
    Private Sub salvaGenpIncluindo(ByRef conection As NpgsqlConnection, ByRef transacao As NpgsqlTransaction)
        ...
        _clGenp.descontopedido = chk_descontopedido.Checked
        ...
        _clGenp.imagemCarne = txt_ImagemCarne.Text : _clGenp.pastaSgbd = txt_pastaSgbd.Text
        _clGenp.logomarcapath = txt_pathLogoMarca.Text
    ...
    Private Sub cbo_ativEmpresa_GotFocus(sender As Object, e As EventArgs) Handles cbo_ativEmpresa.GotFocus
        If cbo_ativEmpresa.DroppedDown = False Then cbo_ativEmpresa.DroppedDown = True
    End Sub

    Private Sub cbo_bancos_GotFocus(sender As Object, e As EventArgs) Handles cbo_blBancos.GotFocus
        If cbo_blBancos.DroppedDown = False Then cbo_blBancos.DroppedDown = True
    End Sub

    Private Sub cbo_optBoleto_GotFocus(sender As Object, e As EventArgs) Handles cbo_optBoleto.GotFocus
        If cbo_optBoleto.DroppedDown = False Then cbo_optBoleto.DroppedDown = True
    End Sub

    Private Sub btn_pastaSgbd_Click(sender As Object, e As EventArgs) Handles btn_pastaSgbd.Click
        pastaSgbd()
    End Sub

    Private Sub pastaSgbd()

        Try
            OpenFolder.ShowDialog()
            If OpenFolder.SelectedPath <> "" Then Me.txt_pastaSgbd.Text = OpenFolder.SelectedPath
        Catch ex As Exception
            Me.txt_pastaSgbd.Text = ""
        End Try

    End Sub

    Private Sub btn_pathLogoMarca_Click(sender As Object, e As EventArgs) Handles btn_pathLogoMarca.Click
        pathLogoMarca()
    End Sub

    Private Sub pathLogoMarca()

        Try
            OpenFile.ShowDialog()
            If OpenFile.FileName <> "" Then Me.txt_pathLogoMarca.Text = OpenFile.FileName
        Catch ex As Exception
            Me.txt_pathLogoMarca.Text = ""
        End Try

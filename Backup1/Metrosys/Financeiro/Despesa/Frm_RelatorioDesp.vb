...
#Region " ** Baixa Mensal - Pagamentos ** "
    ...
    Private Sub executaRelatBaixasMensal1(ByRef sw As Cl_EscreveArquivo, ByVal loja As String, ByRef shouldReturn As Boolean)
        ...
        _mConsulta = "(SELECT ds_grupo, ds_descricao, '' As ""dm_tipo"", '' AS ""dm_subgrupo"",  ds_descricao2, '' AS ""dm_historico"", " & _
        "'' As ""dm_data"", 0.00 As ""dm_valor"", 0 As ""Count"" FROM " & MdlEmpresaUsu._esqEstab & ".desp001 WHERE ds_descricao <> '' AND EXISTS(SELECT " & _
        "dm_grupo FROM " & MdlEmpresaUsu._esqEstab & ".despm002 WHERE ds_grupo = dm_grupo AND (dm_data BETWEEN '" & mDataInicial & "' AND '" & _
        mDataFinal & "'))) UNION (SELECT ds_grupo, ds_descricao, dm_tipo, dm_subgrupo, ds_descricao2, dm_historico, " & _
        "To_Char(dm_data, 'dd/MM/yyyy'), dm_valor, (Select Count(d2.dm_grupo) FROM " & MdlEmpresaUsu._esqEstab & ".despm002 d2 WHERE " & _
        "ds_grupo = d2.dm_grupo AND (d2.dm_data BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "') AND d2.dm_tipo = 'P') As ""Count"" FROM " & MdlEmpresaUsu._esqEstab & ".desp001 JOIN " & _
        MdlEmpresaUsu._esqEstab & ".despm002 ON ds_grupo = dm_grupo WHERE ds_subgrupo = dm_subgrupo AND (dm_data " & _
        "BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "') AND dm_tipo = 'P') ORDER BY ds_grupo ASC, ds_descricao DESC, dm_tipo, dm_subgrupo ASC"

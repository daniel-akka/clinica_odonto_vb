...
Public Class Frm_RelatorioFluxo
        ...
        While dr.Read
            ...
            Try
                dtg_pedidos.Rows.Add(dr(0), dr(1).ToString, dr(2).ToString, Format(CDate(dr(3)), "dd/MM/yyyy"), _
                                 ...
                                 Format(CDbl(dr(8)), "###,##0.00"), dr(9).ToString, dr(10).ToString, dr(11), dr(12), Format(CDbl(dr(13)), "###,##0.00"))
            Catch ex As Exception
                MsgBox(ex.Message)
            End Try
        End While
    ...
    Private Sub preecheDtgPedidosPesquisa(ByVal pesquisa As String, ByVal pesquisa2 As String)
        ...
        End If
        Sqlcomm.Append("order by n1.nt_dtemis limit 5000")
        ...
        Select Case cbo_tiporelatorio.SelectedIndex
            Case 0 'Simples
                If (Me.dtg_pedidos.Rows.Count > _valorZERO) AndAlso (Me.dtg_pedidos.SelectedCells.Count > 0) Then
                    executaEspelho("", "\wged\RelatorioFluxoPedi.txt")
                End If

            Case 1 'Analítico
                If (Me.dtg_pedidos.Rows.Count > _valorZERO) AndAlso (Me.dtg_pedidos.SelectedCells.Count > 0) Then
                    executaEspelho2("", "\wged\RelatorioFluxoPedi.txt")
                End If
            ...
            Case 2 'Arquivo
                ...
                executaRelatorioFluxoArquivo("", "\wged\RelatorioFluxoArquivo.txt")
            ...
            Case 3 'Movimento Caixa
                ...
                executaRelatorioMovimentoCaixa("", "\wged\RelatorioMovimentoCaixa.txt")

            Case 4 'Vendedor
                _frmREf = Me
                frmBuscaVendedor.set_frmRef(Me)
                frmBuscaVendedor.ShowDialog(Me)

                _frmREf = Me
                frmBuscaDataPeriodo.set_frmRef(Me)
                frmBuscaDataPeriodo.ShowDialog(Me)

                '_tipoComisVendedor = _clFuncoes.trazTipoComissaoVendedor(codVendedorRef, MdlEmpresaUsu._codigo, MdlConexaoBD.conectionPadrao)
                '_alqComisVendedor = _clFuncoes.trazAlqComissVendedor(codVendedorRef, MdlEmpresaUsu._codigo, MdlConexaoBD.conectionPadrao)
                executaRelatorioVendedor("", "\wged\RelatorioVendedor.txt")

            Case 5 ' Movimento Diário
                _frmREf = Me
                frmBuscaData.set_frmRef(Me)
                frmBuscaData.ShowDialog(Me)

                executaRelatorioDiario("", "\wged\RelatorioDiario.txt")
...
#Region "Movimento Diário"
    ...
    Public Sub GravCabLojDiario1(ByVal consulta As String, ByRef sw As Cl_EscreveArquivo, ByVal loja As String, _
        ...
        Try
            ...
            sw.EscreveLn(strAux)
            sw.EscreveLn("|                                                                    " & _funcoes.Exibe_StrEsquerda("Data do Caixa: " & Format(CDate(dataRelatorio), "dd/MM/yyyy"), 64) & "|")
            sw.EscreveLn("|                                                                                                                                    |")
            sw.EscreveLn("|                        Relatorio de Caixa Diario Receber                                                                           |")
            sw.EscreveLn("|                                                                          |-----------------------------+---------------------------|")
            sw.EscreveLn("|                                                                          |       R E C E B I D O       |  V E N D A S  D O  D I A  |")
            sw.EscreveLn("|--------+--------------------+-----------+----------+----------+--+--+----+---------+--------+----------+--------+--------+---------|")
            sw.EscreveLn("| Codigo |       Cliente      | Documento |  Emissao |Vencimento| N| P|Dias|  Valor  | Juros  |   Total  | Avista |Entrada |  APrazo |")
            sw.EscreveLn("|--------+--------------------+-----------+----------+----------+--+--+----+---------+--------+----------+--------+--------+---------|")
    ...
    Public Sub GravCabLojDiarioPagDuplic(ByVal consulta As String, ByRef sw As Cl_EscreveArquivo, ByVal loja As String, _
        ...
        Try
            ...
            sw.EscreveLn("|--------+-------------------------------------+-------------+----------+----------+----------+----------|")
            sw.EscreveLn("| Codigo |             Fornecedor              |  Duplicata  | N. Fiscal|  Emissao |Vencimento|  Valor   |")
            sw.EscreveLn("|--------+-------------------------------------+-------------+----------+----------+----------+----------|")
    ...
    Public Sub GravaCabDespCredito()
        ...
        '106 caracteres...
        sw.EscreveLn("+========================================================================================================+")
        sw.EscreveLn("|                      D E S P E S A S / C R E D I T O       N O         C A I X A                       |")
        sw.EscreveLn("|                                                                                                        |")
        sw.EscreveLn("|----------+--------------+---+-----------------------------------------------+------------+-------------|")
    ...
    Public Sub GravCabSLDAnterior(ByVal consulta As String, ByRef sw As Cl_EscreveArquivo, ByVal loja As String, _
        ...
        Try
            ...
            strAux += _funcoes.Exibe_StrDireita("Data do Caixa: " & Format(CDate(dataRelatorio), "dd/MM/yyyy"), 30)
            sw.EscreveLn(strAux)
    ...
    Private Sub executaRelatorioDiario1(ByRef sw As Cl_EscreveArquivo, ByVal loja As String, ByRef shouldReturn As Boolean)
        ...
        While dr.Read
            ...
            mDiasAtrazo = dataDoDiaRef.Subtract(CDate(mDtVencimento)).Days
        ...
        While dr.Read
            ...
            '|00001904987303|VERA LUCIA ALV|31802/02   |11/09/2013|11/11/2013|02|11| 140|    43,60|    0,40|     44,00|        |        |         |
            mStrLinha = "| " & mCodCliente & " | " & _funcoes.Exibe_StrEsquerda(mNomeCliente, 18) & " |"
            mStrLinha += _funcoes.Exibe_StrEsquerda("P-" & mFatura, 11) & "|" & mDtEmiss & "|" & mDtVencimento & "|"
            mStrLinha += mParcelas & "|" & mParcelas & "|    |         |        |          |"
            mStrLinha += _funcoes.Exibe_StrDireita(Format(mAvista, "#,##0.00"), 8) & "|        |         |"
        ...
        While dr.Read
            ...
            mStrLinha = "| " & mCodCliente & " | " & _funcoes.Exibe_StrEsquerda(mNomeCliente, 18) & " |"
            mStrLinha += _funcoes.Exibe_StrEsquerda("P-" & mFatura, 11) & "|" & mDtEmiss & "|          |"
            mStrLinha += "01|" & mParcelas & "|    |         |        |          |        |        |"
            mStrLinha += _funcoes.Exibe_StrDireita(Format(mAPrazo, "##,##0.00"), 9) & "|"
        ...
        While dr.Read
            ...
            mStrLinha += _funcoes.Exibe_StrDireita(Format(total, "###,##0.00"), 10) & "|"
        ...
        Dim mArrayDespCred As Array = Split(mStrBDespCred.ToString, "?")
        Dim mArrayDespCredAux As Array

        For index = 0 To mArrayDespCred.Length - 2
            mArrayDespCredAux = Split(mArrayDespCred(index).ToString, "|")


            mStrLinha = "|" & mData & "|" & _funcoes.Exibe_StrEsquerda(mArrayDespCredAux(0).ToString, 14) & "|041|"
            mStrLinha += "DUPLICATAS PAGAS: " & _funcoes.Exibe_StrEsquerda(mArrayDespCredAux(0).ToString, 29)
            mStrLinha += "|            |" & _funcoes.Exibe_StrDireita("-" & Format(CDbl(mArrayDespCredAux(1).ToString), "###,##0.00"), 13) & "|"

            sw.EscreveLn(mStrLinha)
        Next

        sw.EscreveLn("|-----------------------------------------------------------------------------+------------+-------------|")
        mStrLinha = "|                             Total De Despesas/Credito ..........            |"
        mStrLinha += _funcoes.Exibe_StrDireita(Format(sldAnteriorCX, "###,##0.00"), 12) & "|"
        mStrLinha += _funcoes.Exibe_StrDireita("-" & Format(mSomaTotalDespesas, "###,##0.00"), 13) & "|"
        sw.EscreveLn(mStrLinha)
        sw.EscreveLn("+========================================================================================================+")
        ...
        mStrLinha += _funcoes.Exibe_StrDireita(Format(Round(sldAnteriorCX, 2), "#,###,##0.00"), 21) & "|"
        sw.EscreveLn(mStrLinha)

        sw.EscreveLn("       |---------------------------------------------------------------------+---------------------|")
        mStrLinha = "       | Total do Caixa                  [=]                                 |"
        mStrLinha += _funcoes.Exibe_StrDireita(Format(Round((mSomaTotalDuplic + mSomaAvista + mSomaEntrada + sldAnteriorCX) - mSomaTotalDespesas, 2), "#,###,##0.00"), 21) & "|"
        sw.EscreveLn(mStrLinha)
        ...
        sw.EscreveLn("")
        sw.EscreveLn("             ******************************************************************************")
        sw.EscreveLn("             *              T O T A L   D O   C A I X A            << " & _funcoes.Exibe_StrDireita(Format(Round((mSomaTotalDuplic + mSomaAvista + mSomaEntrada + sldAnteriorCX) - mSomaTotalDespesas, 2), "#,###,##0.00"), 13) & " >>    *")
        sw.EscreveLn("             ******************************************************************************")
        sw.EscreveLn("")
    ...
    Private Sub executaRelatorioDiario(ByVal unidadePC As String, ByVal arqSaida As String)
        ...
        Dim mArqTemp As String = "\wged\TEMPRelatorioDiario.TMP"
...
#Region "Movimento do Caixa"
    ...
    Private Sub executaRelatorioMovCaixa1(ByRef sw As Cl_EscreveArquivo, ByVal loja As String, ByRef shouldReturn As Boolean)
        ...
        _mConsulta = "SELECT CAST( CAST( (SELECT Sum(f_valor) FROM " & MdlEmpresaUsu._esqEstab & ".fatd001 WHERE f_dtpaga BETWEEN '" & _
        mDataInicial & "' AND '" & mDataFinal & "') AS Numeric(15,2)) As Double Precision) As ""Valor"", CAST( CAST( (SELECT Sum(f_juros) " & _
        "FROM " & MdlEmpresaUsu._esqEstab & ".fatd001 WHERE f_dtpaga BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "') AS " & _
        "Numeric(15,2)) As Double Precision) As ""Juros"", CAST( CAST( (SELECT Sum(f_valor) FROM " & MdlEmpresaUsu._esqEstab & ".fatdp02 " & _
        "WHERE f_dtpaga BETWEEN '" & mDataInicial & " ' AND '" & mDataFinal & "') AS Numeric(15,2)) As Double Precision) As ""ValorParcial"""
        comm = New NpgsqlCommand(_mConsulta, oConnBD)
        dr = comm.ExecuteReader
        ...
        '       | Recebimento de Avista           [+]   |                             |                 0,00|
        _mConsulta = "SELECT CAST( CAST( (SELECT Sum(n4_tgeral) FROM " & MdlEmpresaUsu._esqEstab & ".orca4dd JOIN " & MdlEmpresaUsu._esqEstab & _
        ".orca1pp ON nt_orca = n4_nume WHERE (nt_dtemis BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "') AND nt_tipo2 = 'AV' " & _
        "AND (nt_sit BETWEEN 3 AND 5) AND (nt_tiposelecao <> 1 AND nt_tiposelecao <> 2)) AS Numeric(15,2)) As Double Precision) As ""Valor"""
        comm = New NpgsqlCommand(_mConsulta, oConnBD)
        dr = comm.ExecuteReader
        ...
        '       | Recebimento de Entrada          [+]   |                             |                 0,00|
        _mConsulta = "SELECT CAST( CAST( (SELECT Sum(nt_entrada) FROM " & MdlEmpresaUsu._esqEstab & ".orca1pp WHERE " & _
        "(nt_dtemis BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "') AND nt_entrada > 0 " & _
        "AND (nt_sit BETWEEN 3 AND 5) AND (nt_tiposelecao <> 1 AND nt_tiposelecao <> 2)) AS Numeric(15,2)) As Double Precision) As ""Valor"""
        comm = New NpgsqlCommand(_mConsulta, oConnBD)
        dr = comm.ExecuteReader
        ...
        _mConsulta = "SELECT CAST( CAST( (SELECT Sum(f_valor) FROM " & MdlEmpresaUsu._esqEstab & ".fatdp02 WHERE f_dtpaga BETWEEN '" & _
        mDataInicial & "' AND '" & mDataFinal & "') AS Numeric(15,2)) As Double Precision) As ""Valor"", CAST( CAST( (SELECT Sum(f_juros) " & _
        "FROM " & MdlEmpresaUsu._esqEstab & ".fatdp02 WHERE f_dtpaga BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "') AS " & _
        ...
        mStrLinha = "       | Recebimento de Outras Lojas     [+]   |          0,00|          0,00|                 0,00|"
        sw.EscreveLn(mStrLinha)
        '_mConsulta = ""
        'comm = New NpgsqlCommand(_mConsulta, oConnBD)
        'dr = comm.ExecuteReader
        ...
        '    End Try
        '    mStrLinha = "       | Recebimento de Entrada          [+]   |              |              | "
        '    mStrLinha += _clFuncoes.Exibe_StrDireita(Format(mSomaTotal, "##,###,##0.00"), 20) & "|"
        '    sw.EscreveLn(mStrLinha)
        ...
        _mConsulta = "SELECT CAST( CAST( (SELECT Sum(cx_valor) FROM " & MdlEmpresaUsu._esqEstab & ".caixadiario WHERE " & _
        "(cx_data BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "') AND cx_tipo = 'P') AS Numeric(15,2)) As " & _
        "Double Precision) As ""ValorCX"", CAST( CAST( (SELECT Sum(d_valor) FROM fatp001 WHERE d_geno = " & _
        "'" & MdlEmpresaUsu._codigo & " ' AND (d_dtpaga BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "')) AS " & _
        ...
        End While
        dr.Close()
        mSomaDepesas = Round(mSomaTotal, 2)
        mStrLinha = "       | Despesas no Caixa               [-]                                 | "
        mStrLinha += _funcoes.Exibe_StrDireita(Format(Round(mSomaTotal, 2), "##,###,##0.00"), 20) & "| "
        sw.EscreveLn(mStrLinha)

        mStrLinha = "       | Credito  no Caixa               [+]                                 | "
        mStrLinha += _funcoes.Exibe_StrDireita(Format(Round((mSomaReceitas - mSomaDepesas), 2), "##,###,##0.00"), 20) & "| "
        sw.EscreveLn(mStrLinha)
        mStrLinha = "       |---------------------------------------------------------------------+---------------------|"
        sw.EscreveLn(mStrLinha)
        ...
        "OR f_tipo = 'CR' AND (f_emiss BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "'))) As ""Duplicatas1"", " & _
        "(SELECT Sum(f_valor) FROM " & MdlEmpresaUsu._esqEstab & ".fatd001 WHERE f_tipo = 'CH' AND (f_emiss BETWEEN '" & _
        mDataInicial & "' AND '" & mDataFinal & "')) As ""Cheque1"", (SELECT Sum(f_valor) FROM " & MdlEmpresaUsu._esqEstab & _
        ".fatdp02 WHERE (f_tipo = 'NP' OR f_tipo = 'CR' AND (f_emiss BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "')" & _
        ")) As ""Duplicatas2"", (SELECT Sum(f_valor) FROM " & MdlEmpresaUsu._esqEstab & ".fatdp02 WHERE " & _
        "f_tipo = 'CH' AND (f_emiss BETWEEN '" & mDataInicial & "' AND '" & mDataFinal & "')) As ""Cheque2"""
        comm = New NpgsqlCommand(_mConsulta, oConnBD)
        dr = comm.ExecuteReader
...
#End Region
        ...
        Try
            ...
            Try
                ...
                While dr.Read
                    ...
                    strLinha = Format(CDate(dr(0)), "dd/MM/yyyy") & "  "
    ...
    Private Sub gravaCabecalhoFluxoArquivo()
        ...
        oConnBD.ClearPool() : oConnBD.Close() : oConnBD.Dispose()
    ...
    Public Sub GravContasRecebidasFluxoArquivoLaser(ByVal consulta As String, ByRef sw As Cl_EscreveArquivo, _
        ...
        Try
            ...
            While dr.Read
                ...
                Try
                    mDataPaga = Format(CDate(dr(5)), "dd/MM/yyyy")
                Catch ex As Exception
                    mDataPaga = ""
                End Try
                Try
                ...
                Try
                    mDtPagParcial = Format(CDate(dr(11)), "dd/MM/yyyy")
                Catch ex As Exception
                    mDtPagParcial = ""
                End Try

...
Public Class Frm_viewRequisicao
    ...
    Public Shared _frmREf As New Frm_viewRequisicao
    ...
    Private Sub Frm_viewRequisicao_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load

        'relaciona o objeto pd ao procedimento rptGravaTotaisNF
        AddHandler pdRelatPedidos.BeginPrint, AddressOf InicializaRelatorio
    End Sub

    Private Sub Frm_viewRequisicao_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub Frm_viewRequisicao_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Public Function trazRequisicaoBD() As Boolean
        ...
        Try
            sql.Append("SELECT DISTINCT r_req, r_cdport || ' - ' || cad.p_portad AS ""Cliente"", r_data FROM " & MdlEmpresaUsu._esqEstab & ".estm300 ") '4 *, e.e_qtde
            sql.Append("LEFT JOIN cadp001 cad ON cad.p_cod = r_cdport ") 'LEFT JOIN estloja01 e ON e.e_codig = gr_codpr AND e.e_loja = @loja

            If Trim(Me.txt_codPart.Text).Equals("") = False Then

                sql.Append("WHERE r_cdport = '" & Trim(Me.txt_codPart.Text) & "' AND ")

                If IsDate(dtp_inicial.Text) AndAlso IsDate(dtp_final.Text) Then

                    sql.Append("r_data BETWEEN '" & dtp_inicial.Text & "' AND '" & dtp_final.Text & "' ")
                End If

            ElseIf IsDate(dtp_inicial.Text) AndAlso IsDate(dtp_final.Text) Then

                sql.Append("WHERE r_data BETWEEN '" & dtp_inicial.Text & "' AND '" & dtp_final.Text & "' ")
            End If
            sql.Append("ORDER BY r_data DESC ")
            ...
            Else
                ...
                While dr.Read
                    ...
                    dtg_requisicao.Rows.Add(dr(0).ToString, dr(1).ToString, Format(CDate(dr(2)), "dd/MM/yyyy"))
                End While
